...
{
	// NPCs
	private static final int GUIDE = 40005;
	...
	public LastImperialTomb()
	{
		...
		addAttackId(SCARLET1, HALL_ALARM);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			case "FRINTEZZA_INTRO_START":
			{
				...
				startQuestTimer("FRINTEZZA_INTRO_2", 1000, null, player, false);
			...
			case "FRINTEZZA_INTRO_12":
			{
				final Instance world = player.getInstanceWorld();
				final Npc frintezza = world.getParameters().getObject("frintezza", Npc.class);
				broadcastPacket(world, new ExShowScreenMessage(NpcStringId.MOURNFUL_CHORALE_PRELUDE, 2, 5000, true));
			...
			case "SCARLET_SECOND_MORPH_CAMERA_8":
			{
				...
				startQuestTimer("SCARLET_SECOND_MORPH_CAMERA_9", 7000, npc, null, false);
				break;
			}
			case "SCARLET_SECOND_MORPH_CAMERA_9":
			{
				...
				final Location scarletLocation = world.getParameters().getLocation("scarletLocation");
				final Npc activeScarlet2 = addSpawn(SCARLET2, scarletLocation, false, 0, false, world.getId());
				world.setParameter("activeScarlet", activeScarlet2);
				break;
			}
			case "FINISH_CAMERA_1":
			{
				...
				if (world != null)
				{
					...
					startQuestTimer("FINISH_CAMERA_2", 7400, null, player, false);
			...
			case "FINISH_CAMERA_5":
			{
				final Instance world = player.getInstanceWorld();
				enablePlayers(world);
				addSpawn(CUBE, -87767, -152432, -9176, 0, false, 0, false, world.getId());
			...
			case "WARNNING_START":
			{
				...
				if (world != null)
				{
					...
					sm.addString("잠시 후 프린테사가 등장합니다! 준비하세요!");
					world.broadcastPacket(sm);
					startQuestTimer("FRINTEZZA_INTRO_1", 500, null, player, false);
				}
				break;
			}
			case "FIRST_ROOM_KILL":
			{
				final Instance world = player.getInstanceWorld();
				if (world != null)
				{
					world.getNpcs().forEach(Npc::deleteMe);
					startQuestTimer("FIRST_ROOM_START", 1500, null, player, false);
				}
				break;
			}
			case "FIRST_ROOM_SPAWN":
			{
				final Instance world = player.getInstanceWorld();
				if (world != null)
				{
					...
					{
						monster.reduceCurrentHp(1, player, null); // TODO: Find better way for attack
					}
					startQuestTimer("FIRST_ROOM_SPAWN", 25000, npc, player, true);
				}
				break;
			}
			case "FIRST_ROOM_START":
			{
				final Instance world = player.getInstanceWorld();
				if (world != null)
				{
	...
	public String onTalk(Npc npc, Player player)
	{
		if (npc.getId() == GUIDE)
		{
			...
			cancelQuestTimer("WARNNING_START", null, player);
			cancelQuestTimer("FRINTEZZA_INTRO_START", null, player);
		}
		else // Teleport Cube
		{
			final Instance world = npc.getInstanceWorld();
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon, Skill skill)
	{
		...
		{
			if (npc.isScriptValue(0) && (npc.getCurrentHpPercent() <= 80))
			{
				npc.setScriptValue(1);
				startQuestTimer("SCARLET_FIRST_MORPH", 1000, npc, null, false);
			}
			if (npc.isScriptValue(1) && (npc.getCurrentHpPercent() <= 20))
			{
		...
		if (world != null)
		{
			...
			{
				world.setStatus(1);
				startQuestTimer("FIRST_ROOM_SPAWN", 300, npc, attacker, false);
			}
			if (skill != null)
			{
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if (world != null)
		{
			...
			else if (npc.getId() == SCARLET2)
			{
				...
				cancelQuestTimer("PLAY_RANDOM_SONG", npc, null);
				startQuestTimer("DELETE_DEMONS", 500, null, killer, false);
			}
			...
			else if (world.getStatus() >= 2)
			{
				...
				if (killCount <= 0)
				{
					switch (world.getStatus())
					{
						...
						{
							world.setStatus(4);
							world.openCloseDoor(17130043, false);
							startQuestTimer("SECOND_ROOM_START", 5000, npc, killer, false);
							break;
						}
						case 4:
						{
							world.setStatus(5);
							final SystemMessage sm = new SystemMessage(SystemMessageId.S1_N);
							sm.addString("1분 후 프린테사가 등장합니다. 서두르세요!");
							world.broadcastPacket(sm);
							for (int doorId : SECOND_ROUTE_DOORS)
							{
								world.openCloseDoor(doorId, true);
							}
							world.openCloseDoor(17130043, true);
							startQuestTimer("WARNNING_START", 30000, null, killer, false);
