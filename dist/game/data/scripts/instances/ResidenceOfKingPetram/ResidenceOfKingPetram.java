...
public class ResidenceOfKingPetram extends AbstractInstance
{
	...
	private static final SkillHolder EARTH_FURY = new SkillHolder(50059, 2); // When change invul state.
		...
		switch (event)
		{
			...
			case "SPAWN_MINION":
			{
				...
				if (world != null)
				{
					...
					if (!world.getParameters().getBoolean("spawnedMinions", false))
					{
						...
						npc.setInvul(true);
						npc.broadcastSay(ChatType.NPC_SHOUT, "하하, 나는 무적이다!!! 저들을 무찔러라!");
					}
			...
			case "UNSPAWN_MINION":
			{
				...
				if (world != null)
				{
					...
					npc.setInvul(false);
					npc.broadcastSay(ChatType.NPC_SHOUT, "안돼... 이럴수가...");
				}
				break;
			}
			case "SUPPORT_PETRAM":
