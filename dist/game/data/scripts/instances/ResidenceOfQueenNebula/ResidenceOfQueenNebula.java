...
{
	// NPCs
	private static final int RAID_MANAGER = 40005;
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
				...
				if (world != null)
				{
					...
					if (npc.getId() == NEBULA)
					{
						...
						for (int i = 0; i < getRandom(4, 6); i++)
						{
							...
							startQuestTimer("SPAWN_WATER_SLIME", 180000, npc, null);
						}
					}
				}
				break;
			...
			case "CAST_AQUA_RAGE":
			{
				...
				if ((player.isInsideRadius3D(npc, 600)))
				{
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		switch (npc.getId())
		{
			case NEBULA:
			{
				if ((player.isInsideRadius3D(npc, 2000)))
				{
					if (player.getAffectedSkillLevel(AQUA_RAGE) == 1)
					{
					...
					{
						player.stopSkillEffects(AQUA_RAGE_5.getSkill());
					}
				}
				cancelQuestTimer("CAST_AQUA_RAGE", npc, player);
			...
			case WATER_SLIME:
			{
				...
				else if (player.getAffectedSkillLevel(AQUA_RAGE) == 5)
				{
					...
					{
						player.stopSkillEffects(AQUA_RAGE_5.getSkill());
						final Skill skill = SkillData.getInstance().getSkill(AQUA_RAGE, 4);
						skill.applyEffects(player, player);
					}
				}
				break;
