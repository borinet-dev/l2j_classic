...
{
	// NPCs
	private static final int MANAGER = 40012;
	...
	// Items
	private static final int CC = 90919; // 1억 경험치 주문서
	private static final int SCROLL = 90499; // 드래곤의 특별한 열매
	// Doors
		...
		else
		{
			...
			if (world != null)
			{
				...
				else
				{
					...
					{
						teleportPlayerOut(member, world);
					}
				}
			}
		}
			...
			switch (npc.getId())
			{
				...
				case TRUTH:
				{
					...
					else
					{
						...
						if (!isLeader)
						{
							Broadcast.toPlayerScreenMessageS(player, "파티장만이 할 수 있습니다.");
							break;
						}
						for (Player member : members)
						{
							giveItems(member, CC, 1);
							giveItems(member, SCROLL, 2);
						}
					}
					npc.showChatWindow(player);
