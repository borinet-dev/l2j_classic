...
{
	// NPCs
	private static final int MANAGER = 40018;
	...
	{
		super(TEMPLATE_ID);
		addStartNpc(MANAGER, MACHINE, SUPPLIES);
		addFirstTalkId(MANAGER, MACHINE, SUPPLIES);
		addTalkId(MANAGER, MACHINE, SUPPLIES);
		addKillId(BOSSES);
		...
		switch (event)
		{
			...
			case "enter_monster_arena":
			{
				...
				else
				{
					...
					if ((player.getClan() != null) && (player.getParty() != null))
					{
					...
					if (GlobalVariablesManager.getInstance().getLong("CLAN_ARENA_" + player.getClanId(), 0) > System.currentTimeMillis())
					{
						final long reuse = GlobalVariablesManager.getInstance().getLong("CLAN_ARENA_" + player.getClanId(), 0);
						final long remainingTime = (reuse - System.currentTimeMillis()) / 1000;
						final int minutes = (int) ((remainingTime % 3600) / 60);
						player.sendMessage("현재 같은 혈맹의 다른 혈맹원이 입장한 상태입니다. " + minutes + " 후 입장가능합니다.");
						player.sendPacket(new CreatureSay(null, ChatType.BATTLEFIELD, Config.SERVER_NAME_KOR, "현재 같은 혈맹의 다른 혈맹원이 입장한 상태입니다. " + minutes + " 후 입장가능합니다."));
						return null;
					}
					...
					if ((player.getClan() == null) || (player.getParty() == null))
					{
						return "ClanArena-No.htm";
					}
					if (player.getClan().getLevel() < 3)
					{
						player.sendMessage("혈맹 레벨이 3 이상이어야 합니다.");
						return null;
					}
					for (Player member : player.getParty().getMembers())
					{
						...
						{
							player.sendMessage("파티는 혈맹 구성원으로만 구성되어야 합니다.");
							return null;
						}
					}
				}
				final long instantDeleteTime = System.currentTimeMillis() + 1800000;
				enterInstance(player, npc, TEMPLATE_ID);
				GlobalVariablesManager.getInstance().set("CLAN_ARENA_" + player.getClanId(), instantDeleteTime);
				...
				if (world != null)
				{
					...
					{
						GlobalVariablesManager.getInstance().set(GlobalVariablesManager.MONSTER_ARENA_VARIABLE + machine.getScriptValue(), 1); // TODO: 22 for 25 total bosses.
					}
					
					startQuestTimer("machine_talk", 2000, machine, null);
					startQuestTimer("start_countdown", 10000, machine, null);
					startQuestTimer("next_spawn", 10000, machine, null);
				}
				break;
			}
			case "machine_talk":
			{
				...
				startQuestTimer("machine_spawn", 3000, npc, null);
				break;
			}
			case "machine_spawn":
			{
				...
				if (world != null)
				{
					npc.broadcastSay(ChatType.NPC_GENERAL, "잠시 후 보스가 나타납니다!");
				}
				break;
			}
			...
			case "supply_reward":
			{
				...
				if ((world != null) && (npc.getId() == SUPPLIES) && (player.getLevel() > 39) && !REWARDED_PLAYERS.contains(player) && npc.isScriptValue(0))
				{
					...
					// Rare reward.
					if (Rnd.chance(5)) // 5% chance.
					{
						giveItems(player, VALOR_BOX, 1);
					}
					else if (Rnd.chance(10)) // 10% chance.
					{
						giveItems(player, TICKET_M, 1);
					}
					else if (Rnd.chance(15)) // 15% chance.
					{
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (world != null)
		{
			...
			startQuestTimer("remove_supplies", 30000, machine, null);
			
			// On max progress, set last four bosses.
			final int progress = GlobalVariablesManager.getInstance().getInt(GlobalVariablesManager.MONSTER_ARENA_VARIABLE + machine.getScriptValue());
			if (progress > 19) // TODO: 22 for 25 total bosses.
			{
				GlobalVariablesManager.getInstance().set(GlobalVariablesManager.MONSTER_ARENA_VARIABLE + machine.getScriptValue(), 1); // TODO: 22 for 25 total bosses.
			}
			
			if (world.getStatus() > 5) // Finish.
			{
				if (Rnd.chance(50))
				{
					for (Player plr : world.getPlayers())
					{
						plr.sendMessage((world.getStatus() - 1) + "번째 보스를 쓰러트렸습니다!");
						plr.sendMessage("보급품을 선택하여 보상을 획득하세요. 30초 후 다음 보스가 나타납니다!");
						plr.sendPacket(new CreatureSay(null, ChatType.BATTLEFIELD, Config.SERVER_NAME_KOR, (world.getStatus() - 1) + "번째 보스를 쓰러트렸습니다!"));
						plr.sendPacket(new CreatureSay(null, ChatType.BATTLEFIELD, Config.SERVER_NAME_KOR, "보급품을 선택하여 보상을 획득하세요. 30초 후 다음 보스가 나타납니다!"));
					}
					startQuestTimer("next_spawn", 30000, machine, null);
				}
				else
				{
					for (Player plr : world.getPlayers())
					{
						plr.sendMessage((world.getStatus() - 1) + "번째 보스를 쓰러트렸습니다!");
						plr.sendMessage("혈맹 투기장이 종료되었습니다.");
						plr.sendPacket(new CreatureSay(null, ChatType.BATTLEFIELD, Config.SERVER_NAME_KOR, (world.getStatus() - 1) + "번째 보스를 쓰러트렸습니다!"));
						plr.sendPacket(new CreatureSay(null, ChatType.BATTLEFIELD, Config.SERVER_NAME_KOR, "혈맹 투기장이 종료되었습니다."));
						plr.sendPacket(new ExSendUIEvent(plr, false, false, 0, 0, NpcStringId.REMAINING_TIME));
					}
					world.finishInstance(1);
					GlobalVariablesManager.getInstance().set("CLAN_ARENA_" + player.getClanId(), System.currentTimeMillis());
				}
			}
			else // Next boss spawn.
			{
				for (Player plr : world.getPlayers())
				{
					plr.sendMessage((world.getStatus() - 1) + "번째 보스를 쓰러트렸습니다!");
	...
	{
		return npc.getId() == 40018 ? "ClanArena.htm" : +npc.getId() + ".htm";
	}
