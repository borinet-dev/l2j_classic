	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			...
			case "selectGoodFaction":
			{
				...
				player.setTitle(Config.FACTION_GOOD_TEAM_NAME + "진형");
				player.sendMessage("지금부터 " + Config.FACTION_GOOD_TEAM_NAME + " 진형에 속합니다!");
				player.teleToLocation(Config.FACTION_GOOD_BASE_LOCATION);
				broadcastMessageToFaction(Config.FACTION_GOOD_TEAM_NAME, player.getName() + "님의 합류로 " + Config.FACTION_GOOD_TEAM_NAME + "진형은 더욱 굳건해 집니다.");
			...
			case "selectEvilFaction":
			{
				...
				player.setTitle(Config.FACTION_EVIL_TEAM_NAME + "진형");
				player.sendMessage("지금부터 " + Config.FACTION_EVIL_TEAM_NAME + " 진형에 속합니다!");
				player.teleToLocation(Config.FACTION_EVIL_BASE_LOCATION);
				broadcastMessageToFaction(Config.FACTION_EVIL_TEAM_NAME, player.getName() + "님의 합류로 " + Config.FACTION_EVIL_TEAM_NAME + "진형은 더욱 굳건해 집니다.");
				World.addFactionPlayerToWorld(player);
				break;
			}
			case "selectNonFaction":
			{
				if (player.isGood() || player.isEvil())
				{
					World.removeFactionPlayerToWorld(player);
				}
				player.setNone();
				player.getAppearance().setNameColor(Config.FACTION_EVIL_NAME_COLOR);
				player.getAppearance().setTitleColor(Config.FACTION_EVIL_NAME_COLOR);
				player.setTitle(Config.FACTION_EVIL_TEAM_NAME + "진형");
				player.sendMessage("중립을 선택하였습니다!");
				break;
			}
			case "SPEAK":
			{
