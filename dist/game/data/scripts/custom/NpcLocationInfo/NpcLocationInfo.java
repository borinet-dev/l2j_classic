...
public class NpcLocationInfo extends AbstractNpcAI
{
	...
	{
		// 기란
		NPCRADAR.add(30080); // Gatekeeper
		NPCRADAR.add(32478);
		NPCRADAR.add(30300);
		NPCRADAR.add(40005); // 레이드 매니저
		NPCRADAR.add(40006); // 공성 매니저
		NPCRADAR.add(31758); // 도박 매니저
		NPCRADAR.add(34275); // 제련 대장장이 제리스
		NPCRADAR.add(30471); // 대장장이 루피오
		NPCRADAR.add(30512); // 수석 대장장이 쿠스토
		NPCRADAR.add(30012); // 낚시 조합원
		NPCRADAR.add(30081); // 잡화상인 헬베티아
		NPCRADAR.add(30082); // 장신구상인 덴커스
		NPCRADAR.add(30090); // 장신구상인 산드라
		NPCRADAR.add(30091); // 장신구상인 엘리아니
		NPCRADAR.add(30088); // 전사용 방어구 상인 라디아
		NPCRADAR.add(30087); // 마법사용 방어구 상인 포제트
		NPCRADAR.add(30084); // 전사용 무기 상인 그라브너
		NPCRADAR.add(30085); // 마법사용 무기 상인 스타니
		NPCRADAR.add(30511); // 수석 창고지기 게스토
		NPCRADAR.add(30103); // 창고지기 발콘
		NPCRADAR.add(30104); // 창고지기 파만
		NPCRADAR.add(33417); // 판매 대행 매니저 페리나
		// Talking Island
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (Util.isDigit(event))
		{
			...
			if (NPCRADAR.contains(npcId))
			{
				if (npcId == 32478)
				{
					addRadar(player, 83346, 149342, -3400);
					htmltext = "MoveToLoc.htm";
					return htmltext;
				}
				else if (npcId == 30012)
				{
					addRadar(player, 80482, 147708, -3496);
					htmltext = "MoveToLoc.htm";
					return htmltext;
				}
				else if (npcId == 31758)
				{
					addRadar(player, 82899, 147811, -3464);
					htmltext = "MoveToLoc.htm";
					return htmltext;
				}
				int x = 0;
