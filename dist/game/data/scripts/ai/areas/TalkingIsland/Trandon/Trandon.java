...
public class Trandon extends AbstractNpcAI
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (htmltext)
		{
			...
			case "subCertify":
			{
				if ((substrings.length < 2) || player.isMainClassActive())
				{
					return null;
				}
				...
				else
				{
					...
					player.getVariables().set(getSubSkillVariableName(player, level), true);
					giveItems(player, SUB_CERTIFICATE, 1);
					htmltext = getHtm(player, "33490-20.html").replace("%level%", String.valueOf(level));
				}
				break;
			}
			case "learnSubSkill":
			...
			case "deleteSubSkill":
			{
				...
				if (player.isSubClassActive())
				{
					htmltext = "33490-21d.html";
				}
				else if (player.getAdena() < Config.FEE_DELETE_SUBCLASS_SKILLS)
				{
					htmltext = getHtm(player, "33490-22.html").replace("%fee%", Util.formatAdena(Config.FEE_DELETE_SUBCLASS_SKILLS));
				}
	...
	public void onSubChange(OnPlayerSubChange evt)
	{
		...
		if (player.isDualClassActive() || player.isMainClassActive())
		{
