...
public class ClanHallManager extends AbstractNpcAI
{
		...
		if ((clanHall != null) && isOwningClan(player, npc))
		{
			switch (action)
			{
				...
				case "warehouse":
				{
					...
					htmltext = htmltext.replace("%clanAdena%", Util.formatAdena(player.getClan().getWarehouse().getAdena()));
					htmltext = htmltext.replace("%lease%", Util.formatAdena(clanHall.getLease()));
							...
							switch (st.nextToken())
							{
								...
								case "setFunction":
								{
									if (st.countTokens() == 2)
									{
										...
										final int funcLv = Integer.parseInt(st.nextToken());
										final ResidenceFunctionTemplate template = ResidenceFunctionsData.getInstance().getFunction(funcId, funcLv);
										if ((template != null) && (player.getClan().getWarehouse().getAdena() >= template.getCost().getCount()))
										{
											...
											String Cadena = Util.formatAdena(template.getCost().getCount());
											player.getClan().getWarehouse().destroyItemByItemId("Clan Hall Function", template.getCost().getId(), template.getCost().getCount(), player, null);
											player.sendMessage("혈맹창고에서 " + Cadena + " 아데나가 사라졌습니다.");
	...
	private String getFunctionInfo(ResidenceFunction func, String htmltextValue, String name)
	{
		...
		if (func != null)
		{
			...
			htmltext = htmltext.replace("%" + name + "expire%", "다음 인출일: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(func.getExpiration())));
			htmltext = htmltext.replaceAll("%" + name + "deactive%", "[<a action=\"bypass -h Quest ClanHallManager manageFunctions removeFunction confirm " + func.getType().toString() + "\">비활성화</a>]");
		}
		else
		{
