...
public class ClassMaster extends AbstractNpcAI implements IXmlReader
{
	...
	public String onAdvEvent(String eventValue, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			case "thirdclass":
			{
				htmltext = getThirdOccupationChangeHtml(player);
				break;
			}
			case "setclass":
			{
				...
				else if (player.isInCategory(CategoryType.SECOND_CLASS_GROUP) && (player.getLevel() >= 40)) // In retail you can skip first occupation
				{
					...
					if (data != null)
					{
						...
						if (!data.getItemsRequired().isEmpty())
						{
							for (ItemHolder ri : data.getItemsRequired())
							{
								...
								{
									player.sendMessage("전직 비용이 부족합니다.");
	...
	private void showPopupWindow(Player player)
	{
		...
		{
			player.sendPacket(new TutorialShowQuestionMark(7, 0)); // mark id was 1001 - used 2 for quest text
			html = getHtm(player, getSecondOccupationChangeHtml(player));
		}
		else if (player.isInCategory(CategoryType.THIRD_CLASS_GROUP) && (player.getLevel() >= 76))
		{
			player.sendPacket(new TutorialShowQuestionMark(8, 0)); // mark id was 1001 - used 2 for quest text
	...
	private String getClassChangeOptions(Player player, int selectedClassId)
	{
		...
		for (int i = 0; i < _classChangeData.size(); i++)
		{
			...
			sb.append("<tr><td>비용:</td></tr>");
			if (option.getItemsRequired().isEmpty())
			{
				sb.append("<tr><td><font color=LEVEL>무료</font></td></tr>");
			}
			else
			{
