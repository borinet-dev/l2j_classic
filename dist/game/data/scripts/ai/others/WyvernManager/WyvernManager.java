...
public class WyvernManager extends AbstractNpcAI
{
	...
	// Misc
	private static final int CRYSTAL_R_GRADE = 17371;
	private static final int WYVERN = 12621;
	private static final int WYVERN_FEE = 225;
	private static final int STRIDER_LEVEL = 85;
	private static final int GUARDIAN_STRIDER = 16068;
	...
	{
		if (player.isMounted() && (player.getMountLevel() >= STRIDER_LEVEL) && (player.getMountNpcId() == GUARDIAN_STRIDER))
		{
			if (isOwnerClan(npc, player) && (getQuestItemsCount(player, CRYSTAL_R_GRADE) >= WYVERN_FEE))
			{
				takeItems(player, CRYSTAL_R_GRADE, WYVERN_FEE);
				player.dismount();
		...
		switch (event)
		{
			...
			case "RideWyvern":
			{
				if (!Config.ALLOW_WYVERN_ALWAYS)
				{
					...
					{
						player.sendMessage("공성 중에는 와이번을 소환할 수 없습니다.");
						return null;
					}
					final ManagerType type = MANAGERS.get(npc.getId());
					if (type == ManagerType.CASTLE)
