...
/*
 * NPC 대화 AI 클래스입니다. 다양한 NPC가 무작위로 대화를 시작하고 일정 시간마다 무작위 대화를 전파합니다. 
 * 대화는 무작위로 선택된 메시지를 무작위로 선택된 시간마다 브로드캐스트합니다. 
 * 대화 이벤트에 따라 NPC가 적절한 메시지를 브로드캐스트합니다. 
 * 대화가 시작될 때와 NPC가 스폰될 때 대화 타이머가 시작됩니다. 
 * 현재 등록된 NPC: 이브, 츠바키, 리아나, 제이, 알리미, 폴리네, 시비스, 라이아, 라푼젤. 각 NPC에 대한 대화 이벤트 및 메시지는 onAdvEvent 메서드에서 처리됩니다. 
 * 대화 타이머는 onSpawn
