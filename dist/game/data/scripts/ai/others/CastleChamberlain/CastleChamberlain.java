...
public class CastleChamberlain extends AbstractNpcAI
{
	...
	private final void funcReplace(Castle castle, NpcHtmlMessage html, int func, String str)
	{
		...
		else
		{
			...
			html.replace("%" + str + "Reset%", "[<a action=\"bypass -h Quest CastleChamberlain " + str + " 0\">비활성화</a>]");
		}
	}
		...
		switch (st.nextToken())
		{
			...
			case "tax":
			{
				if (isOwner(player, npc))
				{
					...
					html.replace("%tax_rate%", String.valueOf(castle.getTaxPercent(TaxType.BUY)));
					html.replace("%tax_limit%", String.valueOf(maxTax));
					player.sendPacket(html);
				}
				else
				{
			...
			case "set_tax":
			{
				...
				try
				{
					final byte tax = Byte.parseByte(val);
					if ((tax >= 0) && (tax <= maxTax))
					{
				...
				{
					BuilderUtil.sendSysMessage(player, "0에서 " + maxTax + " 사이의 값을 지정해야 합니다.");
					final NpcHtmlMessage html = getHtmlPacket(player, npc, "castlesettaxrate.html");
					html.replace("%tax_rate%", String.valueOf(castle.getTaxPercent(TaxType.BUY)));
					html.replace("%tax_limit%", String.valueOf(maxTax));
					player.sendPacket(html);
				...
				if (isOwner(player, npc) && player.hasClanPrivilege(ClanPrivilege.CS_SET_FUNCTIONS))
				{
					...
					else
					{
						...
						if (door != null)
						{
							...
							else if (castle.getTreasury() >= price)
							{
								castle.addToTreasuryNoTax((-1) * price);
								// player.getClan().getWarehouse().destroyItem("Castle Upgrade Door", Inventory.ADENA_ID, price, null, null);
			...
			case "upgrade_trap_confirm":
			{
				if (isOwner(player, npc) && player.hasClanPrivilege(ClanPrivilege.CS_SET_FUNCTIONS))
				{
					...
					else
					{
						...
						{
							castle.addToTreasuryNoTax((-1) * price);
							// player.getClan().getWarehouse().destroyItem("Castle Upgrade Trap", Inventory.ADENA_ID, price, null, null);
			...
			case "manor":
			{
				...
				{
					player.sendMessage("장원 시스템이 비활성화되었습니다.");
				}
				break;
			}
			case "products":
