...
public class ClanHallAuctioneer extends AbstractNpcAI
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			case "cancelBid":
			{
				...
				long returnadena = clanHallAuction.getClanBid(clan) - (clanHallAuction.getClanBid(clan) / 10);
				...
				htmltext = htmltext.replace("%myBidRemain%", String.valueOf(returnadena));
				break;
			}
			case "cancel":
			{
				...
				// THE_CLAN_DOES_NOT_OWN_A_CLAN_HALL
				long returnadena = clanHallAuction.getClanBid(clan) - (clanHallAuction.getClanBid(clan) / 10);
				player.getClan().getWarehouse().addItem("Clan Hall Auction Outbid", Inventory.ADENA_ID, returnadena, null, null);
			...
			case "rebid":
			{
				if (player.hasClanPrivilege(ClanPrivilege.CH_AUCTION))
				{
					...
					if (clanHallAuction != null)
					{
						...
						htmltext = htmltext.replace("%minBid%", Util.formatAdena(clanHallAuction.getHighestBid()));
						htmltext = htmltext.replace("%myBid%", Util.formatAdena(clanHallAuction.getClanBid(clan)));
						htmltext = htmltext.replace("%auctionEnd%", builder.appendPattern("yyyy/MM/dd HH").appendLiteral("시 ").appendPattern("mm").appendLiteral("분").toFormatter().format(Instant.ofEpochMilli(System.currentTimeMillis() + ClanHallAuctionManager.getInstance().getRemainingTime()).atZone(ZoneId.systemDefault())));
					}
				}
				else
				{
			...
			case "my_auction":
			{
				...
				htmltext = htmltext.replace("%rent%", Util.formatAdena(clanHall.getLease()));
				...
				htmltext = htmltext.replace("%auctionEnd%", builder.appendPattern("yyyy/MM/dd HH").appendLiteral("시 ").appendPattern("mm").appendLiteral("분").toFormatter().format(endTime.atZone(ZoneId.systemDefault())));
	...
	private void processClanHallBypass(Player player, Npc npc, BypassParser parser)
	{
		...
		if (clanHallId > 0)
		{
			...
			if (clanHall != null)
			{
				...
				html.replace("%rent%", Util.formatAdena(clanHall.getLease()));
				html.replace("%grade%", clanHall.getGrade().getGradeValue());
				html.replace("%minBid%", Util.formatAdena(clanHallAuction.getHighestBid()));
				html.replace("%bidNumber%", clanHallAuction.getBidCount());
				html.replace("%auctionEnd%", builder.appendPattern("yyyy/MM/dd HH").appendLiteral("시 ").appendPattern("mm").appendLiteral("분").toFormatter().format(endTime.atZone(ZoneId.systemDefault())));
		...
		else
		{
			...
			else
			{
					...
					.bodyHandler((pages, clanHall, sb) ->
				{
					...
					sb.append(clanHall.getResidenceId());
					sb.append(";</font></a></td><td width=50>");
					sb.append(DateTimeFormatter.ofPattern("MM/dd").format(Instant.ofEpochMilli(System.currentTimeMillis() + ClanHallAuctionManager.getInstance().getRemainingTime()).atZone(ZoneId.systemDefault())));
					sb.append("</td><td width=70 align=right><font color=\"aaffff\">");
					sb.append(Util.formatAdena(auction.getHighestBid()));
	...
	private void processBidBypass(Player player, Npc npc, BypassParser parser)
	{
		...
		if (clanHallId > 0)
		{
			...
			if (clanHall == null)
			{
				player.sendMessage("오류가 발생했습니다. 다시 입찰해주세요.");
				return;
			}
			final Clan clan = player.getClan();
			if (clan == null)
			...
			{
				player.sendMessage("현재 아지트를 보유 중 이므로, 입찰할 수 없습니다.");
				return;
			}
			...
			else
			{
				...
				if (bidder.isPresent())
				{
					...
					{
						final SystemMessage sm = new SystemMessage(SystemMessageId.YOU_WERE_OUTBID_THE_NEW_HIGHEST_BID_IS_S1_ADENA);
						sm.addLong(auction.getHighestBid());
