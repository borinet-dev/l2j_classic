...
public class ProvisionalHalls extends AbstractNpcAI
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equals("buy"))
		{
			...
			synchronized (LOCK)
			{
				...
				{
					player.sendMessage("혈맹을 창설한 군주만 구매가 가능합니다.");
				}
				else if ((player.getClan().getHideoutId() > 0))
				{
					player.sendMessage("이미 아지트가 있습니다.");
				}
				else if ((dayOfWeek != Calendar.SATURDAY) && (dayOfWeek != Calendar.SUNDAY))
				{
					htmltext = getHtm(player, "33359-02.html");
					htmltext = htmltext.replace("%name%", player.getName());
				}
				else if (player.getAdena() < HALL_PRICE)
				{
					player.sendMessage("아지트 구매비용 " + HALL_PRICE + " 아데나가 있어야 합니다.");
				}
				else
				{
	...
	public String onFirstTalk(Npc npc, Player player)
	{
		...
		{
			htmltext = getHtm(player, "33359-01.html");
			htmltext = htmltext.replace("%name%", player.getName());
		}
		htmltext = getHtm(player, "33359-01b.html");
		htmltext = htmltext.replace("%name%", player.getName());
		return htmltext;
	}
