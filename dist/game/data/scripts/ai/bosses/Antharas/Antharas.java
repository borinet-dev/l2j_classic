...
public class Antharas extends AbstractNpcAI
{
	...
	private static final int MIN_PEOPLE = Config.ANTHARAS_MIN_MEMBER;
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			case "enter":
			{
				...
				{
					Broadcast.toPlayerScreenMessageS(player, "현재 안타라스가 사망한 상태이므로 이동할 수 없습니다.");
				}
				...
				{
					Broadcast.toPlayerScreenMessageS(player, "현재 안타라스 둥지 허용가능인원(" + MAX_PEOPLE + "명)을 초과하여 " + zone.getPlayersInside().size() + "명이 참가 중 이므로 입장할 수 없습니다.");
				}
				else if (player.isInParty())
				{
					...
					{
						final List<Player> members = party.getCommandChannel().getMembers();
						final boolean isPartyLeader = party.getCommandChannel().isLeader(player);
						if (!isPartyLeader)
						{
						...
						{
							Broadcast.toPlayerScreenMessageS(player, "현재 연합채널 맴버수(" + members.size() + ")가 레이드 정원을 초과하였습니다. 정원은 " + MAX_PEOPLE + "명이며, " + zone.getPlayersInside().size() + "명이 참가중 입니다.");
						}
						else if (members.size() < MIN_PEOPLE)
						{
						...
						else
						{
							...
							if (getStatus() != IN_FIGHT)
							{
								...
								{
									setStatus(WAITING);
									startQuestTimer("SPAWN_ANTHARAS", Config.ANTHARAS_WAIT_TIME * 60000, _antharas, null);
								}
							}
						}
					}
			...
			case "SPAWN_ANTHARAS":
			{
				...
				_antharas = (GrandBoss) addSpawn(ANTHARAS, 181323, 114850, -7623, 32542, false, 0);
				setStatus(IN_FIGHT);
				_lastAttack = System.currentTimeMillis();
				...
				if ((npc != null) && ((_lastAttack + 900000) < System.currentTimeMillis()))
				{
					...
					for (Creature creature : zone.getCharactersInside())
					{
						if (creature != null)
						{
							...
							{
								creature.teleToLocation(82762 + getRandom(600), 149257 + getRandom(1100), -3464);
							}
			...
			case "SKIP_WAITING":
			{
				if (getStatus() == WAITING)
				{
					...
					player.sendMessage(getClass().getSimpleName() + ": 대기시간을 건너뛰었습니다.");
				}
				else
				{
					player.sendMessage(getClass().getSimpleName() + ": 지금은 대기 시간을 건너뛸 수 없습니다!");
				}
				break;
			}
			case "RESPAWN_ANTHARAS":
			{
				if (getStatus() == DEAD)
				{
					...
					player.sendMessage(getClass().getSimpleName() + ": 안타라스가 부활했습니다!");
				}
				else
				{
					player.sendMessage(getClass().getSimpleName() + ": 안타라스가 살아있는 동안에는 안타라스를 부활시킬 수 없습니다!");
				}
				break;
			}
			case "DESPAWN_MINIONS":
			{
				if (getStatus() == IN_FIGHT)
				{
					...
					{
						player.sendMessage(getClass().getSimpleName() + ": 모든 미니언이 삭제되었습니다!");
					}
				}
				else if (player != null) // Player dont will be null just when is this event called from GM command
				{
					player.sendMessage(getClass().getSimpleName() + ": 지금은 미니언을 디스폰할 수 없습니다!");
				}
				break;
			}
			case "ABORT_FIGHT":
			{
				if (getStatus() == IN_FIGHT)
				{
					...
					for (Creature creature : zone.getCharactersInside())
					{
						if (creature != null)
						{
							if (creature.isPlayer() && !creature.isGM())
							{
								creature.teleToLocation(82762 + getRandom(600), 149257 + getRandom(1100), -3464);
							}
							else if (creature.isNpc())
				...
				{
					player.sendMessage(getClass().getSimpleName() + ": 지금 당장은 전투를 중단할 수 없습니다!");
				}
				break;
			}
			case "MANAGE_SKILL":
