...
public class QueenAnt extends AbstractNpcAI
{
	...
	private static final byte DEAD = 3; // Queen Ant has been killed.
	...
	public String onAggroRangeEnter(Npc npc, Player player, boolean isSummon)
	{
		...
		if (player.getVariables().getInt("여왕개미", 0) == 0)
		{
			Broadcast.toPlayerScreenMessageS(player, "레이드 매니저를 통해서 레이드를 진행해 주시기 바랍니다.");
			player.teleToLocation(81929 + getRandom(50), 149232 + getRandom(50), -3464);
			return null;
		}
		...
		if (!Config.RAID_DISABLE_CURSE && ((character.getLevel() - npc.getLevel()) > 9))
		{
			Skill curse = null;
			if (isMage)
			{
