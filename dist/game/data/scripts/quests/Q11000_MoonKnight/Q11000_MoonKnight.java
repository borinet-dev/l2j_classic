		...
		switch (event)
		{
			...
			case "reward1":
			{
				if (qs.isCond(10))
				{
					...
					if ((player.getLevel() < 28) && canStartQuest(player))
					{
						htmltext = "popup-4.html";
			...
			switch (npc.getId())
			{
				case OL_MAHUM_THIEF:
				{
					if (qs.isCond(2))
					{
						...
						if (getQuestItemsCount(killer, MOLD) >= 10)
						{
							checkHtml(killer);
				...
				case TUREK_ORC_COMMANDER:
				{
					if (qs.isCond(5))
					{
						...
						if ((getQuestItemsCount(killer, ARMOR_TRADE_CONTRACT) >= 1) && (getQuestItemsCount(killer, TUREK_ORC_ORDER) >= 1))
						{
							checkHtml(killer);
							playSound(killer, QuestSound.ITEMSOUND_QUEST_TUTORIAL);
						}
				...
				case TUREK_ORC_INVADER:
				{
					if (qs.isCond(9))
					{
						...
						if (getQuestItemsCount(killer, TUREK_ORC_INVADER_HEAD) >= 10)
						{
							checkHtml(killer);
							playSound(killer, QuestSound.ITEMSOUND_QUEST_TUTORIAL);
						}
