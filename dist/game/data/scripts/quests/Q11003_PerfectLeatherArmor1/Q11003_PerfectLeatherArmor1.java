...
public class Q11003_PerfectLeatherArmor1 extends Quest
{
	...
	public Q11003_PerfectLeatherArmor1()
	{
		...
		addCondCompletedQuest(Q11002_HelpWithTempleRestoration.class.getSimpleName(), "notyet.htm");
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			case "30035-02.htm":
			{
				showOnScreenMsg(player, "잠시 후 렉터 앞으로 이동합니다.", ExShowScreenMessage.TOP_CENTER, 2000);
				player.sendMessage("잠시 후 렉터 앞으로 이동합니다.");
				ThreadPool.schedule(new TeleportTask(player, new Location(-86351, 243087, -3712)), 3000);
				qs.startQuest();
				htmltext = event;
				break;
			}
		...
		if (event.startsWith("reward"))
		{
			if (qs.isCond(4))
			{
				...
				while (tokenizer.hasMoreTokens())
				{
					...
					player.addItem("11003_퀘스트_보상", createditem, null, true);
