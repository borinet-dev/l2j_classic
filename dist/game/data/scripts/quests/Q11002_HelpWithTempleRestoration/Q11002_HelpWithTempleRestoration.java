...
public class Q11002_HelpWithTempleRestoration extends Quest
{
	...
	public void OnPlayerLogin(OnPlayerLogin event)
	{
		...
		if (qs != null)
		{
			if (getQuestItemsCount(event.getPlayer(), WOODEN_POLE) >= 10)
			{
			...
			else if (getQuestItemsCount(event.getPlayer(), WOODEN_DOOR_PANEL) >= 10)
			{
				event.getPlayer().sendPacket(new TutorialShowHtml(getHtm(event.getPlayer(), "popup-2.htm")));
				playSound(event.getPlayer(), QuestSound.ITEMSOUND_QUEST_TUTORIAL);
			}
			else if (getQuestItemsCount(event.getPlayer(), STONE_POWDER) >= 10)
			{
				event.getPlayer().sendPacket(new TutorialShowHtml(getHtm(event.getPlayer(), "popup-2.htm")));
				playSound(event.getPlayer(), QuestSound.ITEMSOUND_QUEST_TUTORIAL);
			}
