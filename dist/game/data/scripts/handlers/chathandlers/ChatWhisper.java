...
public class ChatWhisper implements IChatHandler
{
	...
	public void handleChat(ChatType type, Player activeChar, String target, String text)
	{
		...
		if ((receiver != null) && !receiver.isSilenceMode(activeChar.getObjectId()))
		{
			...
			{
				activeChar.sendMessage("해당 유저는 현재 수감 중 입니다.");
				return;
			}
			if (receiver.isInOlympiadMode())
			{
				activeChar.sendPacket(SystemMessageId.YOU_CANNOT_SEND_A_WHISPER_TO_A_USER_WHO_IS_PARTICIPATING_IN_THE_OLYMPIAD);
				return;
			}
			...
			{
				activeChar.sendMessage("해당 유저는 현재 오프라인 모드입니다.");
				return;
			}
			...
			{
				activeChar.sendMessage("해당 유저와 같은 진영에 속해있지 않아서 귓속말을 할 수 없습니다.");
				return;
			}
