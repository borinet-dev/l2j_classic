...
public class AdminEffects implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String commandValue, Player activeChar)
	{
		...
		if (command.equals("admin_invis_menu") || command.equals("admin_invis") || command.equals("admin_vis"))
		{
			if (!activeChar.isInvisible())
			{
				...
				BuilderUtil.sendSysMessage(activeChar, "투명모드로 전환합니다.");
			}
			else
			{
				...
				activeChar.broadcastUserInfo();
				activeChar.sendPacket(new ExUserInfoAbnormalVisualEffect(activeChar));
				BuilderUtil.sendSysMessage(activeChar, "투명모드를 해제합니다.");
			}
