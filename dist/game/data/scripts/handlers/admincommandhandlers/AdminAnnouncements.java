	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		switch (cmd)
		{
			...
			case "admin_announce_crit":
			{
				...
				else if (cmd.equals("admin_screen"))
				{
					Broadcast.toAllOnlinePlayersOnScreen(announce);
				}
				else
			...
			case "admin_announces":
			{
				...
				switch (subCmd)
				{
					case "add":
					{
						...
						{
							BuilderUtil.sendSysMessage(activeChar, "지연은 10초 미만일 수 없습니다!");
							break;
						}
						// ************************************
						if (!st.hasMoreTokens())
						...
						BuilderUtil.sendSysMessage(activeChar, "공지사항이 성공적으로 추가되었습니다!");
					...
					case "edit":
					{
						...
						if (announce == null)
						{
							BuilderUtil.sendSysMessage(activeChar, "공지사항이 존재하지 않습니다!");
							break;
						}
						if (!st.hasMoreTokens())
						{
						...
						switch (announce.getType())
						{
							...
							case AUTO_NORMAL:
							{
								switch (type)
								{
									...
									default:
									{
										BuilderUtil.sendSysMessage(activeChar, "알림 유형은 AUTO_NORMAL 또는 AUTO_CRITICAL로만 변경할 수 있습니다!");
										return false;
									}
								}
								break;
							...
							case CRITICAL:
							{
								switch (type)
								{
									...
									default:
									{
										BuilderUtil.sendSysMessage(activeChar, "알림 유형은 NORMAL 또는 CRITICAL로만 변경할 수 있습니다!");
										return false;
									}
								}
								break;
						...
						announce.updateMe();
						BuilderUtil.sendSysMessage(activeChar, "공지가 성공적으로 수정되었습니다!");
						return useAdminCommand("admin_announces list", activeChar);
					}
					case "remove":
					{
						...
						{
							BuilderUtil.sendSysMessage(activeChar, "공지가 성공적으로 삭제되었습니다!");
						}
						else
						{
							BuilderUtil.sendSysMessage(activeChar, "공지사항이 존재하지 않습니다!");
					...
					case "restart":
					{
						if (!st.hasMoreTokens())
						{
							...
							BuilderUtil.sendSysMessage(activeChar, "자동 알림이 성공적으로 다시 시작되었습니다.");
							break;
						}
						final String token = st.nextToken();
						if (!Util.isDigit(token))
						...
						if (announce != null)
						{
							...
							{
								BuilderUtil.sendSysMessage(activeChar, "이 옵션은 자동 알림에만 적용됩니다!");
							}
						}
						else
						{
					...
					case "list":
					{
						...
						final PageResult result = PageBuilder.newBuilder(AnnouncementsTable.getInstance().getAllAnnouncements(), 8, "bypass admin_announces list").currentPage(page).bodyHandler((pages, announcement, sb) ->
						{
							...
							{
								sb.append("<td width=60><button action=\"bypass -h admin_announces restart " + announcement.getId() + "\" value=\"재시작\" width=\"60\" height=\"21\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
							}
							else
							{
							...
							{
								sb.append("<td width=60><button action=\"bypass -h admin_announces show " + announcement.getId() + "\" value=\"보기\" width=\"60\" height=\"21\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
								sb.append("<td width=60></td>");
							}
							else
							{
								sb.append("<td width=60><button action=\"bypass -h admin_announces show " + announcement.getId() + "\" value=\"보기\" width=\"60\" height=\"21\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
								sb.append("<td width=60><button action=\"bypass -h admin_announces edit " + announcement.getId() + "\" value=\"수정\" width=\"60\" height=\"21\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
							}
							sb.append("<td width=60><button action=\"bypass -h admin_announces remove " + announcement.getId() + "\" value=\"삭제\" width=\"60\" height=\"21\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
							sb.append("<td width=5></td>");
							sb.append("</tr>");
						}).build();
