...
public class AdminServerInfo implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		if (command.equals("admin_serverinfo"))
		{
			...
			html.replace("%gameTime%", BorinetUtil.timeFormat.format(cal.getTime()));
			html.replace("%dayNight%", GameTimeTaskManager.getInstance().isNight() ? "밤" : "낮");
			html.replace("%geodata%", Config.PATHFINDING > 0 ? "사용" : "미사용");
			html.replace("%serverTime%", BorinetUtil.timeFormat.format(new Date(System.currentTimeMillis())));
	...
	private String getServerUpTime()
	{
		...
		return days + " 일, " + hours + " 시간, " + TimeUnit.MILLISECONDS.toMinutes(time) + " 분";
	}
