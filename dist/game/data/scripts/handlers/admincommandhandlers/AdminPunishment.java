...
public class AdminPunishment implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS =
	{
		...
		"admin_jail",
		"admin_unjail",
		"admin_punishment_allplayer"
	};
	
	private static SimpleDateFormat DATE_FORMATTER = new SimpleDateFormat("MM월 dd일 HH:mm");
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		switch (cmd)
		{
							...
							if (content != null)
							{
								...
								for (PunishmentType type : PunishmentType.values())
								{
									if (PunishmentManager.getInstance().hasPunishment(key, affect, type))
									{
										...
										sb.append("<tr><td><font color=\"LEVEL\">" + type + "</font></td><td>" + expire + "</td><td><a action=\"bypass -h admin_punishment_remove " + name + " " + affect + " " + type + "\">해제하기</a></td></tr>");
									}
								}
			...
			case "admin_punishment_remove":
			{
				...
				GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar.getObjectId() + "]", cmd, affect.name(), name);
				return useAdminCommand("admin_punishment_allplayer", activeChar);
			}
			case "admin_ban_char":
			{
				if (st.hasMoreTokens())
			...
			case "admin_punishment_allplayer":
			{
				String content = HtmCache.getInstance().getHtm(activeChar, "data/html/admin/punishment-allplayer.htm");
				if (content != null)
				{
					final StringBuilder sb = new StringBuilder();
					for (int charId : PunishmentManager.getInstance().getAllPlayerIds())
					{
						for (PunishmentAffect affect : PunishmentAffect.values())
						{
							for (PunishmentType type : PunishmentType.values())
							{
								...
								{
									final long expiration = PunishmentManager.getInstance().getPunishmentExpiration(charId, affect, type);
									String expire = "never";
									if (expiration > 0)
									{
										// Synchronize date formatter since it's not thread safe.
										synchronized (DATE_FORMATTER)
										{
											expire = DATE_FORMATTER.format(new Date(expiration));
										}
									}
									String playerName = PunishmentManager.getInstance().getPlayerName(charId);
									sb.append("<tr><td><font color=LEVEL>").append(playerName).append("</font></td><td>").append(expire).append("</font></td>");
									sb.append("<td><a action=\"bypass -h admin_punishment_remove ").append(playerName).append(" ").append(affect.name()).append(" ").append(type.name()).append("\"><center>출감</center></a></td></tr>");
								}
							}
						}
					}
