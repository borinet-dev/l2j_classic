...
public class AdminMenu implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.startsWith("admin_teleport_character_to_menu"))
		{
			...
			if (data.length == 5)
			{
				...
				if (player != null)
				{
					teleportCharacter(player, new Location(Integer.parseInt(data[2]), Integer.parseInt(data[3]), Integer.parseInt(data[4])), activeChar, "운영자가 당신을 소환하였습니다.");
				}
			}
			showMainPage(activeChar);
		}
		else if (command.startsWith("admin_recall_char_menu"))
		{
			try
			{
				...
				teleportCharacter(player, activeChar.getLocation(), activeChar, "운영자가 당신을 소환하였습니다.");
			}
		...
		else if (command.startsWith("admin_recall_party_menu"))
		{
			try
			{
				...
				if (!player.isInParty())
				{
					BuilderUtil.sendSysMessage(activeChar, "해당 플레이어는 파티가 없습니다.");
					teleportCharacter(player, activeChar.getLocation(), activeChar, "운영자가 당신을 소환하였습니다.");
					return true;
				}
				for (Player pm : player.getParty().getMembers())
				{
					teleportCharacter(pm, activeChar.getLocation(), activeChar, "운영자가 당신의 파티원을 소환하였습니다.");
				}
			}
			catch (Exception e)
			{
		...
		else if (command.startsWith("admin_recall_clan_menu"))
		{
			try
			{
				...
				if (clan == null)
				{
					BuilderUtil.sendSysMessage(activeChar, "타겟은 혈맹이 없습니다.");
					teleportCharacter(player, activeChar.getLocation(), activeChar, "운영자가 당신을 소환하였습니다.");
					return true;
				}
				...
				{
					teleportCharacter(member, activeChar.getLocation(), activeChar, "운영자가 당신의 혈맹원을 소환하였습니다.");
				}
			}
			catch (Exception e)
			{
		...
		else if (command.startsWith("admin_kick_menu"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				if (plyr != null)
				{
					...
					text = plyr.getName() + "님을 추방하였습니다.";
				}
				else
				{
					text = player + "님을 찾을 수 없습니다.";
				}
	...
	private void handleKill(Player activeChar, String player)
	{
		...
		if (player != null)
		{
			...
			{
				target = plyr;
				BuilderUtil.sendSysMessage(activeChar, plyr.getName() + "님을 죽였습니다.");
			}
		}
		if (target != null)
		{
	...
	private void teleportToCharacter(Player activeChar, WorldObject target)
	{
		...
		else
		{
			...
			BuilderUtil.sendSysMessage(activeChar, player.getName() + "님에게 이동하였습니다.");
		}
		showMainPage(activeChar);
	}
