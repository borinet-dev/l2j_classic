...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.startsWith("admin_setreputation"))
		{
			try
			{
				...
				activeChar.setReputation(activeChar.getReputation() + reputation);
		...
		else if (command.startsWith("admin_setclass"))
		{
			try
			{
				...
				if ((ClassId.getClassId(classidval) != null) && (player.getClassId().getId() != classidval))
				{
					...
					if (player.getOriginRace() == Race.ERTHEIA)
					{
						player.getAppearance().setFemale();
					}
					...
					if (player.isInCategory(CategoryType.SIXTH_CLASS_GROUP))
					{
						...
						for (SkillLearn skill : SkillTreeData.getInstance().getRaceSkillTree(player.getOriginRace()))
						{
	...
	private void listCharacters(Player activeChar, int page)
	{
		...
		html.replace("%previous%", page > 0 ? "<button value=\"이전\" action=\"bypass -h admin_show_characters " + previous + " \" width=\"120\" height=22 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">" : "");
		html.replace("%next%", ((result.getPages() > 0) && (next < result.getPages())) ? "<button value=\"다음\" action=\"bypass -h admin_show_characters " + next + " \" width=\"120\" height=22 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">" : "");
	...
	private void gatherCharacterInfo(Player activeChar, Player player, String filename)
	{
		...
		adminReply.replace("%innerIp%", player.getInnerIp());
		adminReply.replace("%protocol%", String.valueOf(player.getClient() != null ? player.getClient().getProtocolVersion() : "NULL"));
		adminReply.replace("%hwid%", player.getHWID() != null ? player.getHWID() : "Unknown");
