...
public class AdminTeleport implements IAdminCommandHandler
{
	...
	private void teleportTo(Player activeChar, String coords)
	{
		try
		{
			...
			BuilderUtil.sendSysMessage(activeChar, "텔레포트 하였습니다: " + coords);
		}
	...
	private void teleportCharacter(Player player, Location loc, Player activeChar)
	{
		if (player != null)
		{
			...
			if (player.isJailed())
			{
				BuilderUtil.sendSysMessage(activeChar, "죄송합니다. " + player.getName() + " 유저는 현재 수감 중 입니다.");
			}
			else
			{
				BuilderUtil.sendSysMessage(activeChar, player.getName() + "님을 소환하였습니다.");
				player.sendMessage("운영자가 당신을 소환하였습니다.");
	...
	private void teleportToCharacter(Player activeChar, WorldObject target)
	{
		...
		else
		{
			...
			BuilderUtil.sendSysMessage(activeChar, player.getName() + " 유저에게 이동하였습니다.");
		}
	}
