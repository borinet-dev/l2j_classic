...
{
	private static final Condition _weaponTypeCondition = new ConditionUsingItemType(WeaponType.BIGSWORD.mask());
	private static final Condition _weaponTypeCondition2 = new ConditionUsingItemType(WeaponType.SWORD.mask());
	...
	{
		if (_weaponTypeCondition.test(effected, effected, skill) //
			|| (_weaponTypeCondition2.test(effected, effected, skill) && _slotCondition.test(effected, effected, skill)))
		{
			switch (_pAtkMode)
			{
				case DIFF:
			...
			switch (_mpAccuracyMode)
			{
				case DIFF:
				{
					effected.getStat().mergeAdd(Stat.ACCURACY_MAGIC, _mpAccuracyAmount);
					break;
				}
				case PER:
				{
					effected.getStat().mergeMul(Stat.ACCURACY_MAGIC, (_mpAccuracyAmount / 100) + 1);
					break;
				}
			}
