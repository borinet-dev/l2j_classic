...
public class BeastSpiritShot implements IItemHandler
{
	...
	public boolean useItem(Playable playable, Item item, boolean forceUse)
	{
		...
		if ((pet != null) && !pet.isChargedShot(shotType))
		{
			// activeOwner.sendPacket(new SystemMessage(SystemMessageId.YOUR_PET_USES_S1).addItemName(item.getId()));
		...
		aliveServitor.forEach(s ->
		{
			...
			{
				// activeOwner.sendPacket(new SystemMessage(SystemMessageId.A_SUMMONED_MONSTER_USES_S1).addItemName(item.getId()));
