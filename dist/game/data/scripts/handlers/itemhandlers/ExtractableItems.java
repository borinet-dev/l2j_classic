...
public class ExtractableItems implements IItemHandler
{
	...
	public boolean useItem(Playable playable, Item item, boolean forceUse)
	{
		...
		// destroy item
		if (!player.destroyItem("사용", item.getObjectId(), 1, player, true))
		{
			return false;
		}
			...
			while (extractedItems.size() < etcitem.getExtractableCountMin())
			{
				for (ExtractableProduct expi : exitems)
				{
					...
					if (Rnd.get(100000) <= expi.getChance())
					{
						...
						{
							final Item newItem = player.addItem("추출획득", expi.getId(), createItemAmount, player, false);
						...
						else
						{
							...
							{
								final Item newItem = player.addItem("추출획득", expi.getId(), 1, player, false);
								if (expi.getMaxEnchant() > 0)
								{
									newItem.setEnchantLevel(Rnd.get(expi.getMinEnchant(), expi.getMaxEnchant()));
									enchantedItems.add(newItem);
