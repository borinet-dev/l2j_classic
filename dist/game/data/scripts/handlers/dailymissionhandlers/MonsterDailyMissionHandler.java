...
public class MonsterDailyMissionHandler extends AbstractDailyMissionHandler
{
	...
	private final int _minLevelMob;
	private final int _maxLevelMob;
	...
	public MonsterDailyMissionHandler(DailyMissionDataHolder holder)
	{
		...
		_minLevelMob = holder.getParams().getInt("minLevelMob", 0);
		_maxLevelMob = holder.getParams().getInt("maxLevelMob", 0);
	...
	private void onAttackableKill(OnAttackableKill event)
	{
		...
		if (_ids.isEmpty() && ((monsterLevel < _minLevelMob) || (monsterLevel > _maxLevelMob)))
		{
			return; // 몬스터 레벨이 범위에 맞지 않으면 처리하지 않습니다.
		}
