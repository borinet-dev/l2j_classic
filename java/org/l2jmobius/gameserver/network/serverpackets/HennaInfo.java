...
public class HennaInfo implements IClientOutgoingPacket
{
	...
	public boolean write(PacketWriter packet)
	{
		...
		packet.writeH(_player.getHennaValue(BaseStat.INT) > 9 ? 9 : _player.getHennaValue(BaseStat.INT)); // equip INT
		packet.writeH(_player.getHennaValue(BaseStat.STR) > 9 ? 9 : _player.getHennaValue(BaseStat.STR)); // equip STR
		packet.writeH(_player.getHennaValue(BaseStat.CON) > 9 ? 9 : _player.getHennaValue(BaseStat.CON)); // equip CON
		packet.writeH(_player.getHennaValue(BaseStat.MEN) > 9 ? 9 : _player.getHennaValue(BaseStat.MEN)); // equip MEN
		packet.writeH(_player.getHennaValue(BaseStat.DEX) > 9 ? 9 : _player.getHennaValue(BaseStat.DEX)); // equip DEX
		packet.writeH(_player.getHennaValue(BaseStat.WIT) > 9 ? 9 : _player.getHennaValue(BaseStat.WIT)); // equip WIT
