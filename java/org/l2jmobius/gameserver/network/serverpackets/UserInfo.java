...
public class UserInfo extends AbstractMaskPacket<UserInfoType>
{
	...
	public boolean write(PacketWriter packet)
	{
		...
		if (containsMask(UserInfoType.BASIC_INFO))
		{
			...
			packet.writeD(_player.getVisualClassId() < 0 ? _player.getBaseTemplate().getClassId().getRootClassId().getId() : _player.getVisualTemplate().getClassId().getRootClassId().getId());
			packet.writeD(_player.getClassId().getId());
			// packet.writeD(_player.getVisualClassId() < 0 ? _player.getClassId().getId() : _player.getVisualClassId());
		...
		{
			packet.writeH(18);
			if (_player.getVisualClassId() != -2)
			{
				packet.writeF(_player.getVisualTemplate().getCollisionRadius());
				packet.writeF(_player.getVisualTemplate().getCollisionHeight());
			}
			else
			{
		...
		{
			packet.writeH(15);
			packet.writeD(Config.ENABLE_VITALITY ? _player.getVitalityPoints() : 0);
