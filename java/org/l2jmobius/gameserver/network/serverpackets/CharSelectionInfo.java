...
public class CharSelectionInfo implements IClientOutgoingPacket
{
	...
	public boolean write(PacketWriter packet)
	{
		...
		packet.writeC(2); // 0=can't play, 1=can play free until level 85, 2=100% free play
		packet.writeD(1); // if 1, Korean client
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (charInfoPackage.isOfflineMod())
			{
				packet.writeS(charInfoPackage.getName() + " - (오프라인 모드 사용 중)"); // Character name
			}
			else
			{
			...
			if ((charInfoPackage.getVisualClassId() < 0))
			{
				packet.writeD(charInfoPackage.getBaseClassId());
			}
			else
			{
				packet.writeD(charInfoPackage.getVisualClassId());
			}
			...
			packet.writeD(Config.ENABLE_VITALITY ? charInfoPackage.getVitalityPoints() : 0); // Vitality
	...
	private static CharSelectInfoPackage restoreChar(ResultSet chardata) throws Exception
	{
		...
		final int visual_Classid = chardata.getInt("visual_classid");
		...
		charInfopackage.setVisualClassId(visual_Classid);
		
		charInfopackage.setDeleteTimer(deletetime);
		charInfopackage.setLastAccess(chardata.getLong("lastAccess"));
		charInfopackage.setNoble(chardata.getInt("nobless") == 1);
		charInfopackage.setOfflineMod(chardata.getInt("online") == 2 ? true : false);
