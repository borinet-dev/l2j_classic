...
public class RequestDestroyItem implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (player.hasPet() && (itemToRemove.getItemType() == EtcItemType.PET_COLLAR))
		{
			player.sendMessage("펫이 소환된 상태에서는 파쇄할 수 없습니다.");
			return;
		}
		...
		{
			player.sendMessage("저주받은 무기는 파쇄할 수 없습니다.");
			return;
		}
		...
		if (itemId == Config.LUNA)
		{
			player.sendMessage(itemName + "는 파쇄할 수 없습니다.");
			return;
		}
		...
		{
			player.sendMessage("이 아이템은 사용할 수 없습니다.");
			return;
		}
		...
		final Item removedItem = player.getInventory().destroyItem("파괴", itemToRemove, count, player, null);
		if (removedItem == null)
		{
			return;
		}
