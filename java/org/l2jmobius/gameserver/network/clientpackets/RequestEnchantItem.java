...
public class RequestEnchantItem implements IClientIncomingPacket
{
	...
	{
		int skillLevel = Math.min(enchantLevel, 20);
		final int skillId = 23096 + skillLevel;
	...
	public void run(GameClient client)
	{
		...
		if (player.getInventory().destroyItem("사용", scroll.getObjectId(), 1, player, item) == null)
		{
		...
		if ((support != null) && (player.getInventory().destroyItem("사용", support.getObjectId(), 1, player, item) == null))
		{
			player.sendPacket(SystemMessageId.INCORRECT_ITEM_COUNT_2);
		...
		synchronized (item)
		{
			...
			switch (resultType)
			{
				...
				case SUCCESS:
				{
					...
					EnchantRecord.getInstance().deleteItemRecovery(player, item.getObjectId());
					
					player.sendPacket(new EnchantResult(EnchantResult.SUCCESS, item));
					EventDispatcher.getInstance().notifyEventAsync(new OnItemEnchant(player, item));
					...
					if (item.getEnchantLevel() >= 1)
					{
						...
						{
							player.sendMessage("축하합니다! +" + item.getEnchantLevel() + " " + item.getItemName() + " 강화에 성공하였습니다.");
							// player.sendPacket(new CreatureSay(null, ChatType.BATTLEFIELD, Config.SERVER_NAME_KOR, "축하합니다! +" + item.getEnchantLevel() + " " + item.getItemName() + " 강화에 성공하였습니다."));
							Broadcast.toAllOnlinePlayersOnScreen(player.getName() + "님이 +" + item.getEnchantLevel() + " " + item.getItemName() + " 강화에 성공하였습니다.");
				...
				case FAILURE:
				{
					...
					else
					{
						...
						if (scrollTemplate.isBlessed() || scrollTemplate.isBlessedDown() || ((supportTemplate != null) && supportTemplate.isDown()) || ((supportTemplate != null) && supportTemplate.isBlessed()))
						{
							...
							{
								player.sendMessage("인챈트 값이 1 감소합니다.");
								item.setEnchantLevel(item.getEnchantLevel() - 1);
							}
							else // blessed enchant - clear enchant value
							{
								EnchantRecord.addEnchantRecord(item.getObjectId(), item.getId(), player.getObjectId(), item.getName(), item.getEnchantLevel(), true);
						...
						{
							EnchantRecord.addEnchantRecord(item.getObjectId(), item.getId(), player.getObjectId(), item.getName(), item.getEnchantLevel(), false);
							// enchant failed, destroy item
							if (player.getInventory().destroyItem("삭제", item, player, null) == null)
							{
