...
public class UseItem implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			player.sendMessage("감옥에서는 사용할 수 없습니다.");
			return;
		}
	...
	private void reuseData(Player player, Item item, long remainingTime)
	{
		...
		if (hours > 0)
		{
			player.sendMessage(item.getItemName() + "의 재사용 시간이 " + hours + "시간 " + minutes + "분 " + seconds + "초 남았습니다.");
		}
		else if (minutes > 0)
		{
			player.sendMessage(item.getItemName() + "의 재사용 시간이 " + minutes + "분 " + seconds + "초 남았습니다.");
		}
		else
		{
			player.sendMessage(item.getItemName() + "의 재사용 시간이 " + seconds + "초 남았습니다.");
		}
	}
