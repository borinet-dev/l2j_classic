...
public class RequestAcquireSkill implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		switch (_skillType)
		{
			case CLASS:
			{
				if (checkNormalSkillLearn(player, trainer, s))
				{
	...
	private boolean checkNormalSkillLearn(Player player, Npc trainer, SkillLearn skillLearn)
	{
		if (skillLearn != null)
		{
			if ((skillLearn.getSkillId() == _id) && (skillLearn.getSkillLevel() == _level))
			{
				...
				{
					player.sendPacket(SystemMessageId.YOU_DO_NOT_MEET_THE_SKILL_LEVEL_REQUIREMENTS);
					Util.handleIllegalPlayerAction(player, "Player " + player + ", level " + player.getLevel() + " is requesting skill Id: " + _id + " level " + _level + " without having minimum required level, " + skillLearn.getGetLevel() + "!", IllegalActionPunishmentType.NONE);
					return false;
				}
