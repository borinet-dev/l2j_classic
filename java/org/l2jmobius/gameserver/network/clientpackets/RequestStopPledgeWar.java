...
public class RequestStopPledgeWar implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (clan == null)
		{
			player.sendMessage("혈맹이름이 잘못되었습니다.");
		...
		{
			player.sendMessage("해당 혈맹과 전쟁 중이 아닙니다.");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		{
			member.sendMessage(clan.getName() + " 혈맹에 선포된 전쟁을 철회하였습니다.");
			member.sendPacket(new PledgeReceiveWarList(member.getClan(), 0));
			member.broadcastUserInfo();
		}
		
		for (Player member : clan.getOnlineMembers(0))
		{
			member.sendMessage(playerClan.getName() + " 혈맹이 전쟁 선포를 철회하였습니다.");
			member.sendPacket(new PledgeReceiveWarList(member.getClan(), 0));
			member.broadcastUserInfo();
		}
	}
