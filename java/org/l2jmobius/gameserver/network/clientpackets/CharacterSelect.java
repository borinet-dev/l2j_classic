...
public class CharacterSelect implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			ConfirmDlg confirmDlg = new ConfirmDlg("지금 접속하시려는 서버는 테스트서버입니다.\n2초 후 접속합니다.");
			client.sendPacket(confirmDlg);
			try
			{
				Thread.sleep(2000);
			}
		...
		if (client.getPlayerLock().tryLock())
		{
			try
			{
				...
				if (client.getPlayer() == null)
				{
					...
					if ((info.getAccessLevel() < 100) && Config.ONLY_CREATE_CHARACTER)
					{
						final NpcHtmlMessage msg = new NpcHtmlMessage();
						msg.setFile(null, "data/html/mods/OnlyCreateChar/OnlyCreateChar-1.htm");
					...
					{
						long endTimes = PunishmentManager.getInstance().getPunishmentExpiration(info.getObjectId(), PunishmentAffect.CHARACTER, PunishmentType.BAN);
						...
						String reason = PunishmentManager.getInstance().getReason(info.getObjectId(), PunishmentAffect.CHARACTER, PunishmentType.BAN);
						
						final NpcHtmlMessage msg = new NpcHtmlMessage();
						msg.setFile(null, "data/html/mods/Ban.htm");
						msg.replace("%times%", times);
						msg.replace("%reason%", reason);
						msg.replace("%endtime%", TimeUtil.formatTimes(delay, false));
						client.sendPacket(msg);
						client.close(ServerClose.STATIC_PACKET);
						return;
					}
					
					if (PunishmentManager.getInstance().hasPunishment(client.getAccountName(), PunishmentAffect.ACCOUNT, PunishmentType.BAN) //
					...
					{
						final NpcHtmlMessage msg = new NpcHtmlMessage();
						msg.setFile(null, "data/html/mods/LowAccess.htm");
						client.sendPacket(msg);
						client.close(ServerClose.STATIC_PACKET);
						return;
					}
