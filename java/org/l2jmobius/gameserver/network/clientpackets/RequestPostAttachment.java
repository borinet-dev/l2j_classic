...
public class RequestPostAttachment implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			PreparedStatement rss = con.prepareStatement("SELECT id, messageId FROM items_mail WHERE delivered = 1 AND received_time IS NULL AND charId=" + player.getObjectId());
			ResultSet donate = rss.executeQuery();
			while (donate.next())
			{
				PreparedStatement rssb = con.prepareStatement("UPDATE items_mail SET received_time = '" + BorinetUtil.dataDateFormat.format(new Date(System.currentTimeMillis())) + "' WHERE messageId = " + _msgId + " AND id = " + donate.getInt("id"));
				rssb.executeQuery();
			}
			
			PreparedStatement rsb = con.prepareStatement("SELECT id, messageId FROM items_reward_mail WHERE delivered = 1 AND received_time IS NULL AND charId=" + player.getObjectId());
			ResultSet minigame = rsb.executeQuery();
			while (minigame.next())
			{
				PreparedStatement rsc = con.prepareStatement("UPDATE items_reward_mail SET received_time = '" + BorinetUtil.dataDateFormat.format(new Date(System.currentTimeMillis())) + "' WHERE messageId = " + _msgId + " AND id = " + minigame.getInt("id"));
				rsc.executeQuery();
			}
		}
