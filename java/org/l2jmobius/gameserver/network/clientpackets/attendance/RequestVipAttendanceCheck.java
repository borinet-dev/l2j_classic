...
public class RequestVipAttendanceCheck implements IClientIncomingPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (player.getUptime() < (Config.ATTENDANCE_REWARD_DELAY * 60 * 1000))
		{
			...
			msg.addInt((int) ((Config.ATTENDANCE_REWARD_DELAY) - (player.getUptime() / 60000)));
		...
		if (isRewardAvailable)
		{
			...
			// Save date and index.
			player.setAttendanceInfo(rewardIndex + 1, nextReward.getTimeInMillis());
			// Add items to player.
			player.addItem("출석보상", reward, player, true);
			// Send message.
			final SystemMessage msg = new SystemMessage(SystemMessageId.YOU_VE_RECEIVED_YOUR_ATTENDANCE_REWARD_FOR_DAY_S1);
