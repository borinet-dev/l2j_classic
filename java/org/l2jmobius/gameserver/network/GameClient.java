...
public class GameClient extends ChannelInboundHandler<GameClient>
{
	...
	protected void channelRead0(ChannelHandlerContext ctx, IIncomingPacket<GameClient> packet)
	{
		...
		{
			// LOGGER.log(Level.WARNING, "Exception for: " + toString() + " on packet.run: " + packet.getClass().getSimpleName());
		}
	}
	...
	public CharacterDeleteFailType markToDeleteChar(int characterSlot)
	{
		...
		{
			final int clanId = CharInfoTable.getInstance().getClanIdById(objectId);
			if (clanId > 0)
			{
	...
	public static void deleteCharByObjId(int objid)
	{
		...
		CharInfoTable.getInstance().removeName(objid);
	...
	public String toString()
	{
		try
		{
			...
			switch (state)
			{
				case CONNECTED:
				{
					return "[IP: " + (address == null ? "연결끊김" : address.getHostAddress()) + "]";
				}
				case AUTHENTICATED:
				{
					return "[계정: " + _accountName + " - IP: " + (address == null ? "연결끊김" : address.getHostAddress()) + "]";
				}
				case ENTERING:
				case IN_GAME:
				{
					return "[캐릭터: " + (_player == null ? "disconnected" : _player.getName() + "[" + _player.getObjectId() + "]") + " - 계정: " + _accountName + " - IP: " + (address == null ? "연결끊김" : address.getHostAddress()) + "]";
				}
				default:
				{
