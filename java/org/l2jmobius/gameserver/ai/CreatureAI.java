...
import static org.l2jmobius.gameserver.ai.CtrlIntention.AI_AUTO_FOLLOW;
...
public class CreatureAI extends AbstractAI
{
	...
	@Override
	protected void onIntentionAutoFollow(Creature target)
	{
		if (getIntention() == AI_INTENTION_REST)
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the Player actor
		...
		// Set the Intention of this AbstractAI to AI_INTENTION_FOLLOW
		changeIntention(AI_AUTO_FOLLOW, target);
		
		// Create and Launch an AI Follow Task to execute every 1s
		String[] leaders = BorinetUtil.getInstance().getAutoFollow((Player) _actor);
		int range = Integer.valueOf(leaders[1]);
		startFollow(target, range);
	}
