...
public class TimeAmountInterpreter
{
	...
	{
		return consolidate(timeAmount, timeUnit, timeUnit, DAYS, "0 " + timeUnit.name().toLowerCase(Locale.KOREAN));
	}
	...
	private static <T extends Appendable & CharSequence> T appendConsolidated(T textBuilder, long amount, TimeUnit timeUnit, TimeUnit minConsolidationUnit, TimeUnit maxConsolidationUnit, String noTimeUsedIndicator)
	{
		try
		{
			...
			for (int i = maxConsolidationUnit.ordinal(); i >= minConsolidationUnit.ordinal(); --i)
			{
				...
				final String unit = activeUnit.name().toLowerCase(Locale.KOREAN);
			...
			if (textBuilder.length() == len)
			{
				return (T) textBuilder.append(noTimeUsedIndicator).append(' ').append(minConsolidationUnit.name().toLowerCase(Locale.KOREAN));
			}
		}
		catch (IOException e)
		{
