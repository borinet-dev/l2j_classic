...
public class Party extends AbstractPlayerGroup
{
	...
	public void removePartyMember(Player player, PartyMessageType type)
	{
		if (_members.contains(player))
		{
			...
			if (type == PartyMessageType.EXPELLED)
			{
				if (pet != null)
				{
					if ((pet.getId() < 1568) || (pet.getId() > 1573))
					{
						if (player.isAffectedBySkill(AAR_OF_SEDUCTION))
						{
							player.getEffectList().stopSkillEffects(SkillFinishType.REMOVED, AAR_OF_SEDUCTION.getSkill());
						}
					}
				}
				else
	...
	{
		final double addExp = addExpValue * Config.EXP_AMOUNT_MULTIPLIERS[player.getClassId().getId()];
		final double addSp = addSpValue * Config.SP_AMOUNT_MULTIPLIERS[player.getClassId().getId()];
		double xp = addExp;
		double sp = addSp;
