...
public class Skill implements IIdentifiable
{
	...
	private final boolean _isEventCallSkill;
	...
	public Skill(StatSet set)
	{
		...
		_name = SkillNameTable.getInstance().getskillnameKor(_id);
		...
		{
			abnormalTime = Config.SKILL_DURATION_LIST.get(_id) * 60;
		}
		...
		_isEventCallSkill = set.getBoolean("eventCallSkill", false);
	...
	{
		return _stayAfterDeath || _isNecessaryToggle;
	}
	
	public boolean isEventCallSkill()
	{
		return _isEventCallSkill;
	}
	...
	public boolean checkCondition(Creature creature, WorldObject object, boolean sendMessage)
	{
		...
		if (!checkConditions(SkillConditionScope.GENERAL, creature, object) || !checkConditions(SkillConditionScope.TARGET, creature, object))
		{
			if (!((creature == object) && isBad())) // Self targeted bad skills should not send a message.
			{
				...
				sm.addSkillName(_id, _level, 0);
				creature.sendPacket(sm);
				player.getVariables().remove(_itemConsumeId + "_재사용시간");
			}
			return false;
		}
	...
	{
		return !isPassive() && !isToggle() && !_isDebuff && !isHeroSkill() && !isGMSkill() && !(isStatic() && (getId() != CommonSkill.CARAVANS_SECRET_MEDICINE.getId())) && _canBeDispelled;
	}
