...
public class SkillCaster implements Runnable
{
	...
	{
		if (creature.isRaid() && creature.giveRaidCurse() && (caster.getLevel() > (creature.getLevel() + 8)))
		{
			if (skill.isBad() || ((creature.getTarget() == caster) && ((Attackable) creature).getAggroList().containsKey(caster)))
			{
				if (((Npc) creature).getTemplate().isSevenSignsParty() || ((Npc) creature).getTemplate().isSevenSignsSolo())
				{
					return;
				}
	...
	public static void triggerCast(Creature creature, WorldObject target, Skill skill, Item item, boolean ignoreTargetType)
	{
		...
		{
			LOGGER.log(Level.WARNING, "Failed simultaneous cast: [" + target.getName() + "]");
		}
	}
	...
	public static boolean checkUseConditions(Creature caster, Skill skill, SkillCastingType castingType)
	{
		...
		if (caster.isPlayer())
		{
			...
			if (player.isOnEvent())
			{
				...
				{
					player.sendMessage("이벤트 중에는 " + skill.getName() + "스킬을 사용할 수 없습니다.");
					return false;
				}
