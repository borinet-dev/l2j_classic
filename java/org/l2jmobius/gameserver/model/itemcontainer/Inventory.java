...
public abstract class Inventory extends ItemContainer
{
	...
	{
		final int bodySlot = (int) item.getTemplate().getBodyPart();
		...
		else
		{
			switch (bodySlot)
			{
				case ItemTemplate.SLOT_LR_HAND:
				{
					...
					if ((lh != null) && lh.isArmor() && (lh.getArmorItem().getItemType() == ArmorType.SIGIL))
					{
						setPaperdollItem(PAPERDOLL_LHAND, null);
					}
				...
				{
					final Item rHandItem = getPaperdollItem(PAPERDOLL_RHAND);
					...
					else if (newItem.getItemType() == EtcItemType.LURE)
					{
						...
						if (rHandItemTemplate.getItemType() != WeaponType.FISHINGROD)
						{
							return;
						}
						if (!getOwner().isPlayer())
				...
				default:
					LOGGER.warning("unknown body slot:" + bodySlot + " for item id: " + item.getId());
					return;
			}
		}
	...
	public Item findEquippedLure()
	{
		Item lure = null;
		final Player player = (Player) getOwner();
		if (player.getVariables().getInt("LastLure", 0) > 0)
		{
			for (Item item : _items)
			{
				if (item.isEtcItem() && (item.getEtcItem().getItemType() == EtcItemType.LURE) && (item.getObjectId() == player.getVariables().getInt("LastLure", 0)))
				{
					lure = item;
					break;
				}
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					...
					try
					{
						...
						{
							addItem("아이템정렬", item, getOwner().getActingPlayer(), null);
						}
						else
						{
							addItem(item);
					...
					{
						LOGGER.warning("[" + rs.getInt("item_id") + "] - 아이템 데이터가 없습니다. 소유주: " + player.getName());
					}
				}
			}
			refreshWeight();
	...
	public void reEquipFirstFoundItem()
	{
		int slot;
		for (Item item : _paperdoll)
		{
			if ((item == null) //
				|| (item.getLocationSlot() == Inventory.PAPERDOLL_GLOVES) //
				|| (item.getLocationSlot() == Inventory.PAPERDOLL_FEET) //
				|| (item.getLocationSlot() == Inventory.PAPERDOLL_CHEST) //
				|| (item.getLocationSlot() == Inventory.PAPERDOLL_LEGS) //
				|| (item.getLocationSlot() == Inventory.PAPERDOLL_HEAD))
			{
				continue;
			}
			slot = item.getLocationSlot();
			final List<Item> unequipped = unEquipItemInSlotAndRecord(slot);
