...
public class ClanWar
{
	...
	public ClanWar(Clan attacker, Clan attacked)
	{
		...
		if (Config.ALT_IGNORE_FOR_WAR && (attacked.getCastleId() != 0))
		{
			_state = ClanWarState.MUTUAL;
			sm = new SystemMessage(SystemMessageId.A_CLAN_WAR_WITH_CLAN_S1_HAS_STARTED_THE_CLAN_THAT_CANCELS_THE_WAR_FIRST_WILL_LOSE_500_CLAN_REPUTATION_ANY_CLAN_THAT_CANCELS_THE_WAR_WILL_BE_UNABLE_TO_DECLARE_A_WAR_FOR_1_WEEK_IF_YOUR_CLAN_MEMBER_GETS_KILLED_BY_THE_OTHER_CLAN_XP_DECREASES_BY_1_4_OF_THE_AMOUNT_THAT_DECREASES_IN_THE_HUNTING_GROUND);
			sm.addString(attacked.getName());
			attacker.broadcastToOnlineMembers(sm);
			attacker.getVariables().set("StartWar", true);
			
			String text = attacker.getName() + " 혈맹이 혈맹전을 선포하였습니다. 성을 소유하였기에 자동으로 혈맹전이 시작됩니다!";
			attacked.broadcastToOnlineMembers(text);
			
			sm = new SystemMessage(SystemMessageId.A_CLAN_WAR_WITH_CLAN_S1_HAS_STARTED_THE_CLAN_THAT_CANCELS_THE_WAR_FIRST_WILL_LOSE_500_CLAN_REPUTATION_ANY_CLAN_THAT_CANCELS_THE_WAR_WILL_BE_UNABLE_TO_DECLARE_A_WAR_FOR_1_WEEK_IF_YOUR_CLAN_MEMBER_GETS_KILLED_BY_THE_OTHER_CLAN_XP_DECREASES_BY_1_4_OF_THE_AMOUNT_THAT_DECREASES_IN_THE_HUNTING_GROUND);
			sm.addString(attacker.getName());
			attacked.broadcastToOnlineMembers(sm);
			attacked.getVariables().set("StartWar", true);
	...
	public void onKill(Player killer, Player victim)
	{
		...
		else if ((_state == ClanWarState.BLOOD_DECLARATION) && (victimClan.getId() == _attackerClanId) && (victim.getReputation() >= 0))
		{
			...
			if (killCount >= 5)
			{
				...
				killerClan.getVariables().set("StartWar", true);
				
				sm = new SystemMessage(SystemMessageId.A_CLAN_WAR_WITH_CLAN_S1_HAS_STARTED_THE_CLAN_THAT_CANCELS_THE_WAR_FIRST_WILL_LOSE_500_CLAN_REPUTATION_ANY_CLAN_THAT_CANCELS_THE_WAR_WILL_BE_UNABLE_TO_DECLARE_A_WAR_FOR_1_WEEK_IF_YOUR_CLAN_MEMBER_GETS_KILLED_BY_THE_OTHER_CLAN_XP_DECREASES_BY_1_4_OF_THE_AMOUNT_THAT_DECREASES_IN_THE_HUNTING_GROUND);
				sm.addString(killerClan.getName());
				victimClan.broadcastToOnlineMembers(sm);
				victimClan.getVariables().set("StartWar", true);
