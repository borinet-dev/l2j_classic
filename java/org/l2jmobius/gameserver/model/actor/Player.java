...
public class Player extends Playable
{
	...
	private static final String INSERT_CHARACTER = "INSERT INTO characters (account_name,charId,char_name,level,maxHp,curHp,maxCp,curCp,maxMp,curMp,face,hairStyle,hairColor,sex,exp,sp,reputation,fame,raidbossPoints,pvpkills,pkkills,clanid,race,classid,deletetime,cancraft,title,title_color,online,clan_privs,wantspeace,base_class,nobless,power_grade,vitality_points,visual_classid,createDate) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,reputation=?,fame=?,raidbossPoints=?,pvpkills=?,pkkills=?,clanid=?,race=?,classid=?,deletetime=?,title=?,title_color=?,online=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,nobless=?,power_grade=?,subpledge=?,lvl_joined_academy=?,apprentice=?,sponsor=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,bookmarkslot=?,vitality_points=?,visual_classid=?,language=?,faction=?,pccafe_points=? WHERE charId=?";
	...
	private ScheduledFuture<?> _deathPenaltyTask;
	...
	private static final SkillHolder AAR_OF_SEDUCTION = new SkillHolder(23168, 1); // Custom Ghost Walking
	/** The list of sub-classes this character has. */
	private final Map<Integer, SubClassHolder> _subClasses = new ConcurrentHashMap<>();
	private final Map<Integer, Integer> _symbolId = new ConcurrentHashMap<>();
	...
	private final boolean _isNone = false;
	...
	private BroochJewel _activeSapphireJewel = null;
	...
	// Auto Play
	private final AtomicBoolean _autoPlayStart = new AtomicBoolean();
		...
		else
		{
			...
			if (item.isEquipped())
			{
				...
				if (getCurrentHp() >= (getMaxHp() * 0.9))
				{
					ThreadPool.schedule(() ->
					{
						setCurrentHp(getCurrentHp() + (getMaxHp() * (healItem(item))));
					}, 200);
				}
	...
	private double healItem(Item item)
	{
		...
		else if (item.getId() == 91062)
		{
			healItem = 0.08;
		}
		else if (item.getId() == 91063)
		{
			healItem = 0.10;
		}
		else if ((item.getId() == 91064) || (item.getId() == 91065) || (item.getId() == 91066))
		{
			healItem = 0.12;
		}
		else if (item.getId() == 91067)
		{
			healItem = 0.14;
		}
		else if (item.getId() == 91068)
		{
			healItem = 0.16;
		}
		else if ((item.getId() == 91069) || (item.getId() == 91070) || (item.getId() == 91071))
		{
			healItem = 0.18;
		}
		else if (item.getId() == 91072)
		{
			healItem = 0.2;
		}
		else if (item.getId() == 91073)
		{
			healItem = 0.22;
		}
		else if (item.getId() == 91074)
		{
			healItem = 0.24;
		}
		else if (item.getId() == 91075)
		{
			healItem = 0.3;
		}
		return healItem;
	...
	private void regiveTemporarySkills()
	{
		...
		_inventory.reEquipFirstFoundItem();
	}
	...
	public Race getRace()
	{
		if (getVisualClassId() >= 0)
		{
			// return PlayerTemplateData.getInstance().getTemplate(_visual_classId).getRace();
			return PlayerTemplateData.getInstance().getTemplate(getVisualClassId()).getRace();
		}
		if (isMainClassActive())
		{
		...
		if (count > 0)
		{
			...
			if (sendMessage && ((!isCastingNow() && item.hasExImmediateEffect()) || !item.hasExImmediateEffect()))
			{
				if (count > 1)
				{
					...
					{
						final SystemMessage sm = new SystemMessage(SystemMessageId.YOU_HAVE_OBTAINED_S1_ADENA);
						sm.addLong(count);
						sendPacket(sm);
					}
					else if (itemId == 41000)
					{
						final SystemMessage sm = new SystemMessage(SystemMessageId.YOU_HAVE_OBTAINED_S1_LUNA);
						sm.addLong(count);
						sendPacket(sm);
					}
					else
	...
	public boolean destroyItemByItemIdInWareHouse(String process, int itemId, long count, WorldObject reference, boolean sendMessage)
	{
		final Item item = _warehouse.getItemByItemId(itemId);
		final long quantity = (count < 0) && (item != null) ? item.getCount() : count;
		if ((item == null) || (item.getCount() < quantity) || (quantity <= 0) || (_warehouse.destroyItemByItemId(process, itemId, quantity, this, reference) == null))
		{
			if (sendMessage)
			{
				sendPacket(SystemMessageId.INCORRECT_ITEM_COUNT);
	...
	public void doAutoLoot(Attackable target, int itemId, long itemCount)
	{
		...
		{
			addAdena("루팅", itemCount, target, true);
		}
		else
		{
			addItem("루팅", itemId, itemCount, target, true);
		}
	}
	...
	public boolean doDie(Creature killer)
	{
		...
		if (getReputation() < 0)
		{
			int toDecrease = (Config.KARMA_LOST_MIN / 10) + (getPkKills() * Config.KARMA_SP_DIVIDER);
			int setKarma = getReputation() + Rnd.get(toDecrease / 2, toDecrease * 2);
			setReputation(setKarma > 0 ? 0 : setKarma);
		}
		...
		if ((!isInsideZone(ZoneId.PVP) || (pk == null)) && (!isGM() || Config.KARMA_DROP_GM))
		{
			...
			if ((dropPercent > 0) && (Rnd.get(100) < dropPercent))
			{
				...
				for (Item itemDrop : _inventory.getItems())
				{
					...
					{
						dropItem("사망드랍", itemDrop, killer, true);
	 ...
	 * @param fame
	 */
	public void restoreExp(double restorePercent, boolean fame)
	{
	...
	private boolean createDb()
	{
			...
			PreparedStatement statement = con.prepareStatement(INSERT_CHARACTER))
		{
			...
			statement.setInt(23, getOriginRace().ordinal());
			...
			statement.setInt(36, getVisualClassId());
	...
	private static Player restore(int objectId)
	{
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				if (rset.next())
				{
					...
					player.setVisualClassId(rset.getInt("visual_classid"));
		...
		{
			LOGGER.log(Level.SEVERE, "Failed loading character.");
		}
		return player;
	}
	...
	public String getEmail()
	{
		...
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT e_mail FROM accounts WHERE login = '" + getAccountName() + "'"))
		{
	...
	private void storeCharBase()
	{
			...
			PreparedStatement statement = con.prepareStatement(UPDATE_CHARACTER))
		{
			...
			statement.setInt(46, getVisualClassId());
	...
	public boolean checkHenna(int henna_Id)
	{
		int symbolId = 0;
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement statement = con.prepareStatement(RESTORE_CHAR_HENNAS))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				while (rset.next())
				{
					...
					if (symbolId > 0)
					{
						_symbolId.put(getObjectId(), symbolId);
						
						if (henna_Id == _symbolId.get(getObjectId()))
						{
							_symbolId.remove(getObjectId());
							return true;
						}
					}
				}
		...
		{
			LOGGER.log(Level.SEVERE, "캐릭터 문신정보 체크 오류: " + getName());
		}
	...
	public boolean canLogout()
	{
		...
		if (isSubclassLocked())
		{
			LOGGER.warning("Player " + getName() + " tried to logout during class change.");
			sendMessage("현재 로그아웃 할 수 없습니다. 잠시 후 다시 시도해주세요.");
			return false;
		}
		...
		// if (isInBoat())
		// {
		// sendMessage("정기선 탑승 상태에서는 로그아웃 할 수 없습니다. 잠시 후 다시 시도해주세요.");
		// return false;
		// }
		
		if (isInOlympiadMode())
		{
			sendMessage("올림피아드 참가중에는 로그아웃 할 수 없습니다.");
			return false;
		}
		...
		if (isCaptchaActive)
		{
			sendMessage("보안문자 입력 중 에는 로그아웃 할 수 없습니다.");
			return false;
		}
		
		if (hasPet())
		{
			if ((_pet.getInventory() != null) && (_pet.getInventory().getSize() > 0))
			{
				setPetItems();
				return true;
			}
			if ((_pet.getId() < 1568) || (_pet.getId() > 1573))
			{
				if (isAffectedBySkill(AAR_OF_SEDUCTION))
				{
					getEffectList().stopSkillEffects(SkillFinishType.REMOVED, AAR_OF_SEDUCTION.getSkill());
					return true;
				}
			}
		}
	...
	public boolean canRestart()
	{
		...
		if (isSubclassLocked())
		{
			LOGGER.warning("Player " + getName() + " tried to restart during class change.");
			sendMessage("현재 리스타트 할 수 없습니다. 잠시 후 다시 시도해주세요.");
			return false;
		}
		...
		// if (isInBoat())
		// {
		// sendMessage("정기선 탑승 상태에서는 리스타트 할 수 없습니다. 잠시 후 다시 시도해주세요.");
		// return false;
		// }
		
		if (isInOlympiadMode())
		{
			sendMessage("올림피아드 참가중에는 리스타트 할 수 없습니다.");
			return false;
		}
		
		if (AttackStanceTaskManager.getInstance().hasAttackStanceTask(this) && !(isGM() && Config.GM_RESTART_FIGHTING))
		{
			sendPacket(SystemMessageId.YOU_CANNOT_RESTART_WHILE_IN_COMBAT);
			return false;
		}
		...
		if (isCaptchaActive)
		{
			sendMessage("보안문자 입력 중 에는 리스타트 할 수 없습니다.");
			return false;
		}
	...
	public boolean useMagic(Skill skill, Item item, boolean forceUse, boolean dontMove)
	{
		...
		if (getVariables().getBoolean("자동따라가기", false))
		{
			if (usedSkill.hasEffectType(EffectType.AGGRESSION) || usedSkill.hasEffectType(EffectType.HATE) || usedSkill.hasEffectType(EffectType.HP_DRAIN) || usedSkill.hasEffectType(EffectType.MAGICAL_ATTACK) || usedSkill.hasEffectType(EffectType.PHYSICAL_ATTACK) || usedSkill.hasEffectType(EffectType.LETHAL_ATTACK) || usedSkill.hasEffectType(EffectType.MUTE) || usedSkill.hasEffectType(EffectType.ROOT) || usedSkill.hasEffectType(EffectType.SLEEP) || usedSkill.hasEffectType(EffectType.STEAL_ABNORMAL) || usedSkill.hasEffectType(EffectType.PHYSICAL_ATTACK_HP_LINK) || usedSkill.hasEffectType(EffectType.DMG_OVER_TIME) || usedSkill.hasEffectType(EffectType.DMG_OVER_TIME_PERCENT) || (usedSkill.getTargetType() == TargetType.ENEMY) || (usedSkill.getTargetType() == TargetType.GROUND) || usedSkill.isDebuff())
			{
				sendMessage("자동 따라가기 모드에서는 공격/마법 스킬을 사용할 수 없습니다.");
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
			}
		}
	...
	public boolean isMage()
	{
		...
		if (CategoryData.getInstance().isInCategory(CategoryType.DIVISION_ENCHANTER, classId.getId()) //
			|| CategoryData.getInstance().isInCategory(CategoryType.DIVISION_SUMMON, classId.getId()) //
			|| CategoryData.getInstance().isInCategory(CategoryType.DIVISION_WIZARD, classId.getId()) //
			|| CategoryData.getInstance().isInCategory(CategoryType.DIVISION_HEAL, classId.getId()))
		{
			return true;
		}
		return false;
	...
	public BroochJewel getActiveSapphireJewel()
	{
		return _activeSapphireJewel;
	}
	
	public void setActiveSapphireJewel(BroochJewel jewel)
	{
		_activeSapphireJewel = jewel;
	}
	...
	{
		BorinetUtil.getInstance().AutoItem(this, value);
	...
	public boolean canDuel()
	{
		...
		if (isMounted() || (isInBoat() && isOnBoat()))
		{
	...
	public void setNoble(boolean value)
	{
		...
		else
		{
			...
			if (getInventory().getInventoryItemCount(7694, -1) >= 1)
			{
				destroyItemByItemId("노블레스 티아라 회수", 7694, -1, this, true);
			}
		}
		_noble = value;
		sendSkillList();
	...
	public boolean isMainClassActive()
	{
		return _classIndex == 0;
	}
	...
	public void setActiveClass(int classIndex)
	{
		...
		_subclassLock = true;
		if (getDeathPenaltyLevel() > 0)
		{
			penaltyLogout();
		}
		
		final BuffInfo buff = getEffectList().isEventBuffs();
		if (buff != null)
		{
			getVariables().set("EventCallBuff", true);
		}
		else
		{
			getVariables().remove("EventCallBuff");
		}
		
		getVariables().set("CURRENT_HP_" + getClassIndex(), (int) getCurrentHp());
		getVariables().set("CURRENT_MP_" + getClassIndex(), (int) getCurrentMp());
		getVariables().set("CURRENT_CP_" + getClassIndex(), (int) getCurrentCp());
		...
		try
		{
			...
			for (Skill oldSkill : getAllSkills())
			{
				if (oldSkill.isBad())
				{
					removeAutoSkill(oldSkill.getId());
				}
				else
				{
					removeAutoBuff(oldSkill.getId());
				}
			...
			// stopAllEffects();
			getEffectList().stopEffects(info -> !info.getSkill().isNecessaryToggle(), true, false);
			...
			BorinetUtil.getInstance().subChangeHideSkill(this);
	...
	/**
	 * Expertise of the Player (None=0, D=1, C=2, B=3, A=4, S=5, S80=6, S84=7, R=8, R95=9, S99=10)
	...
	public void teleToLocation(ILocational loc, boolean allowRandomOffset)
	{
		if ((_vehicle != null) && !_vehicle.isTeleporting() && (getBoat() != null))
		{
			...
			setVehicle(null);
			setInVehiclePosition(null);
			setOnBoat(false);
		}
		
		if (isInBoat() || (getBoat() != null))
		{
			setVehicle(null);
			setInVehiclePosition(null);
			setOnBoat(false);
	...
	{
		getStat().addExpAndSp(addToExp, addToSp, false, false);
	}
	...
	public int getInventoryLimit()
	{
		...
		else if (getOriginRace() == Race.DWARF)
		{
	...
	{
		int getDP = getVariables().getInt("DEATH_PENALTY_" + getClassIndex(), 0);
		if (getDP != 0)
		{
			return getDP;
		}
	...
	public void setDeathPenalty(Creature killer)
	{
		if (killer == null)
		{
		...
		if (isResurrectSpecialAffected() || isLucky() || isBlockedFromDeathPenalty() || isInsideZone(ZoneId.PVP) || isInsideZone(ZoneId.SIEGE) || isOnEvent())
		{
			return;
		}
		
		if (((killer.isNpc() && ((Npc) killer).getTemplate().isDeathPenalty()) || (Rnd.get(1, 100) <= Config.DEATH_PENALTY_CHANCE)) && (!killer.isPlayable() || (getReputation() < 0)))
		{
	...
	{
		int nextLv = getDeathPenaltyLevel() + 1;
		if (nextLv > 15)
		{
			nextLv = 15;
		}
		
		final Skill skill = SkillData.getInstance().getSkill(5076, nextLv);
		if (skill != null)
		{
			castDeathPenaltyBuff(nextLv);
			startPenalty(true);
			getVariables().set("DEATH_PENALTY_" + getClassIndex(), nextLv);
			getVariables().set("DEATH_PENALTY_START_" + getClassIndex(), System.currentTimeMillis());
			sendPacket(new SystemMessage(SystemMessageId.YOU_VE_BEEN_AFFLICTED_BY_SHILEN_S_BREATH_LEVEL_S1).addInt(nextLv));
		}
	}
	
	public void decreaseDeathPenalty()
	{
		int nextLv = getDeathPenaltyLevel() - 1;
		final Skill skill = SkillData.getInstance().getSkill(5076, nextLv);
		if (nextLv > 0)
		...
		else
		{
			...
			getVariables().remove("DEATH_PENALTY_" + getClassIndex());
			getVariables().remove("DEATH_PENALTY_START_" + getClassIndex());
			getVariables().remove("DEATH_PENALTY_PLAYTIME_" + getClassIndex());
			getVariables().remove("DEATH_PENALTY_RESTART_" + getClassIndex());
			stopSkillEffects(SkillFinishType.REMOVED, 5076);
			sendPacket(new EtcStatusUpdate(this));
			sendPacket(SystemMessageId.SHILEN_S_BREATH_HAS_BEEN_PURIFIED);
		}
	}
	
	public void castDeathPenaltyBuff(int level)
	{
		final Skill deathPenalty = SkillData.getInstance().getSkill(5076, level);
		addSkill(deathPenalty, true);
		sendSkillList();
		deathPenalty.applyEffects(this, this, false, 3600);
		sendPacket(new EtcStatusUpdate(this));
	}
	
	public void startPenalty(boolean start)
	{
		if (_deathPenaltyTask != null)
		{
			_deathPenaltyTask.cancel(false);
		}
		
		long totalDelay = Config.DEATH_PENALTY_REMOVE_TIME * 60 * 1000;
		
		if (!start)
		{
			long beforePlayTime = getVariables().getLong("DEATH_PENALTY_PLAYTIME_" + getClassIndex(), 0);
			totalDelay = totalDelay - beforePlayTime;
			getVariables().set("DEATH_PENALTY_RESTART_" + getClassIndex(), System.currentTimeMillis());
		}
		
		_deathPenaltyTask = ThreadPool.schedule(() -> decreaseDeathPenalty(), totalDelay);
	}
	
	private void penaltyLogout()
	{
		long penaltyStartTime = getVariables().getLong("DEATH_PENALTY_RESTART_" + getClassIndex(), 0);
		if (penaltyStartTime == 0)
		{
			penaltyStartTime = getVariables().getLong("DEATH_PENALTY_START_" + getClassIndex(), 0);
		}
		
		long elapsedTime = System.currentTimeMillis() - penaltyStartTime;
		long previousPlayTime = getVariables().getLong("DEATH_PENALTY_PLAYTIME_" + getClassIndex(), 0);
		getVariables().set("DEATH_PENALTY_PLAYTIME_" + getClassIndex(), previousPlayTime + elapsedTime);
	}
	...
	public void teleportBookmarkGo(int id)
	{
		...
		if (!BorinetUtil.getInstance().canTeleport(this))
		{
			return;
		}
	...
	public boolean teleportBookmarkCondition(int type)
	{
		...
		else if ((type == 1) && (isInsideZone(ZoneId.SIEGE) || isInsideZone(ZoneId.CLAN_HALL) || isInsideZone(ZoneId.JAIL) || isInsideZone(ZoneId.CASTLE) || isInsideZone(ZoneId.NO_SUMMON_FRIEND) || isInsideZone(ZoneId.PVP) || isInsideZone(ZoneId.FORT)))
		{
			sendMessage("해당 지역은 자유 텔레포트를 기억할 수 없는 지역입니다.");
			// sendPacket(SystemMessageId.YOU_CANNOT_USE_MY_TELEPORTS_TO_REACH_THIS_AREA);
			return false;
		}
		else if (isOnEvent())
		{
			sendMessage("이벤트 중에는 자유 텔레포트를 이용할 수 없습니다.");
			return false;
		}
		else if (isInsideZone(ZoneId.NO_BOOKMARK) || (isOnBoat() && isInBoat()) || isInAirShip() || isInsideZone(ZoneId.PVP))
		{
			if (type == 0)
			{
	...
	private boolean isSendingInfo = false;
	...
	public boolean isAllowedToEnchantSkills()
	{
		...
		if ((isInBoat() && isOnBoat()) || isInAirShip())
		{
			return false;
		}
		return true;
	...
	public int checkBirthDay()
	{
		...
		{
			int days = _createDate.get(Calendar.DAY_OF_MONTH) - now.get(Calendar.DAY_OF_MONTH);
			return days;
		}
		return -1;
	}
	 ...
	 * 종족 외형변경 소스 시작
	 */
	private int _visual_classId;
	
	public int getVisualClassId()
	{
		return _visual_classId;
	}
	
	public void setVisualClassId(int val)
	{
		_visual_classId = val;
	}
	
	public PlayerTemplate getVisualTemplate()
	{
		return PlayerTemplateData.getInstance().getTemplate(getVisualClassId());
	}
	// 종족 외형변경 소스 끝
	...
	{
		final long endDate = PremiumManager.getInstance().getPremiumExpiration(getAccountName());
		if (endDate > 0)
		{
			_isPremium = true;
		}
	...
	public AttendanceInfoHolder getAttendanceInfo()
	{
		...
		if (System.currentTimeMillis() > receiveDate)
		{
			canBeRewarded = true;
	...
	public void setAttendanceInfo(int rewardIndex, long nextReward)
	{
		if (Config.ATTENDANCE_REWARDS_SHARE_ACCOUNT)
		{
			getAccountVariables().set(ATTENDANCE_DATE_VAR, nextReward);
			getAccountVariables().set(ATTENDANCE_INDEX_VAR, rewardIndex);
		}
		else
		{
			getVariables().set(ATTENDANCE_DATE_VAR, nextReward);
	...
	{
		if ((getLevel() >= 40) && (getClassId().level() > 1))
		{
			tryLoadSpirits();
	...
	public long getLuna()
	{
		long lunaCount = getInventory().getInventoryItemCount(Config.LUNA, -1);
		return lunaCount;
	}
	...
	public void updateExpertise()
	{
		...
		else if ((level < 40) && (getExpertiseLevel() > 1))
		{
			experLevel = 1;
		}
		else if ((level < 52) && (getExpertiseLevel() > 2))
		{
			experLevel = 2;
		}
		else if ((level < 61) && (getExpertiseLevel() > 3))
		{
			experLevel = 3;
		}
		else if ((level < 76) && (getExpertiseLevel() > 4))
		{
			experLevel = 4;
		}
		else if ((level < 80) && (getExpertiseLevel() > 5))
		{
			experLevel = 5;
		}
		else if ((level < 84) && (getExpertiseLevel() > 6))
		{
			experLevel = 6;
		}
		else if ((level < 86) && (getExpertiseLevel() > 7))
		{
			experLevel = 8;
		}
		else if ((level < 89) && (getExpertiseLevel() > 9))
		{
			experLevel = 9;
		}
		
		addSkill(SkillData.getInstance().getSkill(239, experLevel), true);
		sendSkillList();
		sendPacket(new EtcStatusUpdate(this));
	}
	
	// 스탯포인트
	private int _Stat_point;
	private int _Str;
	private int _Dex;
	private int _Con;
	private int _Mpw;
	...
	public void statUpdate(String stat, int count, boolean insert)
	{
		if (!insert)
		{
			try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE character_stats SET " + stat + " = '" + count + "' WHERE charId = '" + getObjectId() + "'"))
			{
		...
		{
			try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement statement = con.prepareStatement("INSERT INTO character_stats (char_name, charId) VALUES ('" + getName() + "', '" + getObjectId() + "')"))
			{
				statement.executeQuery();
			}
			catch (Exception e)
	...
	public void statUpdateA(int count)
	{
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement statement = con.prepareStatement("UPDATE accounts SET stats = '" + count + "' WHERE login = '" + getAccountName() + "'"))
		{
			statement.executeQuery();
		}
		catch (Exception e)
	...
	{
		int[] point = CustomStats.getInstance().StatCheck(this);
		_Stat_point = point[0];
		
		return _Stat_point;
	}
	
	public int getStatPointA()
	{
		int point = CustomStats.getInstance().getStatsAcc(this);
		_Stat_point = point;
		
		return _Stat_point;
	}
	
	public void setStatPoint(int val)
	{
		statUpdate("point", val, false);
		_Stat_point = val;
	}
	...
	public void setMpw(int val)
	{
		CustomStats.Mpw(this, val);
		_Mpw = val;
	}
	...
	public void setWit(int val)
	{
		CustomStats.Wit(this, val);
		_Wit = val;
	}
	...
	public void setMen(int val)
	{
		CustomStats.Men(this, val);
		_Men = val;
	}
	
	// 스펙 포인트
	private int _spec_point;
	private int _spec_1;
	private int _spec_2;
	private int _spec_3;
	private int _spec_4;
	...
	public void specUpdate(String stat, int count, boolean insert)
	{
		if (!insert)
		{
			try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE character_spec SET " + stat + " = '" + count + "' WHERE charId = '" + getObjectId() + "'"))
			{
				statement.executeQuery();
			}
			catch (Exception e)
		...
		{
			try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement statement = con.prepareStatement("INSERT INTO character_spec (char_name, charId, point) VALUES ('" + getName() + "', '" + getObjectId() + "', '10')"))
			{
				statement.executeQuery();
			}
			catch (Exception e)
	...
	public void specUpdateA(int count)
	{
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement statement = con.prepareStatement("UPDATE accounts SET spec = '" + count + "' WHERE login = '" + getAccountName() + "'"))
		{
			statement.executeQuery();
		}
		catch (Exception e)
	...
	public int getSpecPointA()
	{
		int point = CustomStats.getInstance().getSpecAcc(this);
		_Stat_point = point;
	...
	{
		int[] point = CustomStats.getInstance().specCheck(this);
		_spec_point = point[0];
		
		return _spec_point;
	}
	
	public void setSpecPoint(int val)
	{
		specUpdate("point", val, false);
		_spec_point = val;
	}
	...
	public void setSpec1(int val)
	{
		CustomStats.spec1(this, val);
		_spec_1 = val;
	}
	...
	public void setSpec2(int val)
	{
		CustomStats.spec2(this, val);
		_spec_2 = val;
	}
	...
	public void setSpec3(int val)
	{
		CustomStats.spec3(this, val);
		_spec_3 = val;
	}
	...
	public void setSpec4(int val)
	{
		CustomStats.spec4(this, val);
		_spec_4 = val;
	}
	...
	public void setSpec5(int val)
	{
		CustomStats.spec5(this, val);
		_spec_5 = val;
	}
	...
	public void setSpec6(int val)
	{
		CustomStats.spec6(this, val);
		_spec_6 = val;
	}
	...
	public void setSpec7(int val)
	{
		CustomStats.spec7(this, val);
		_spec_7 = val;
	}
	...
	public void setSpec8(int val)
	{
		CustomStats.spec8(this, val);
		_spec_8 = val;
	}
	...
	public String getInnerIp()
	{
		String innerIp = "";
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT pcIp FROM accounts WHERE login = ?"))
		{
			statement.setString(1, getAccountName());
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{
				innerIp = rset.getString("pcIp");
			}
		}
		catch (Exception e)
		{
	...
	public void setPetItems()
	{
		...
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT object_id, item_id, count FROM items WHERE owner_id = ? AND (loc = 'PET' OR loc = 'PET_EQUIP')"))
		{
			statement.setInt(1, getObjectId());
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{
				objectId = rset.getInt("object_id");
				itemId = rset.getInt("item_id");
				itemCount = rset.getInt("count");
				
				if (objectId > 0)
				{
					delPetItems();
					addItem("", itemId, itemCount, this, false);
				}
			}
		}
		catch (Exception e)
	...
	public void delPetItems()
	{
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement ps = con.prepareStatement("DELETE FROM items WHERE owner_id = ? AND (loc = 'PET' OR loc = 'PET_EQUIP')"))
		{
			ps.setInt(1, getObjectId());
			ps.execute();
		}
	...
	{
		return _autoPlayStart.get();
	}
	
	public void setAutoPlay(boolean value)
	{
		_autoPlayStart.set(value);
	}
	...
	public Integer getSweeperSkillId()
	{
		return 42;
	}
	...
	public void addAutoSkill(Integer skillId)
	{
		getAutoSkills().add(skillId);
	}
	
	public void removeAutoSkill(Integer skillId)
	{
	...
	public void updateCaptchaCount(int count)
	{
		_captchaCount = count;
	}
	
	private final int[] captchaAnswerNumbers = new int[4]; // 보안문자 답변
	private final int[] captchaQuestNumbers = new int[4]; // 보안문자 출력
	private final int[] captchaNumberPads = new int[10]; // 보안문자 입력패드
	
	private int captchaAnswerCount = 0; // 보안문자 입력횟수
	
	// 보안문자 초기화
	public void clearCaptcha()
	{
		captchaAnswerCount = 0;
		for (int i = 0; i < 4; i++)
		{
			captchaAnswerNumbers[i] = 10; // 초기값으로 설정
			captchaQuestNumbers[i] = 10; // 초기값으로 설정
		}
		for (int i = 0; i < 10; i++)
		{
			captchaNumberPads[i] = 10; // 초기값으로 설정
		}
	}
	
	// 보안문자 답변 설정
	public void updateCaptchaAnswerNumber(int index, int answer)
	{
		if ((index >= 0) && (index < 4))
		{
			captchaAnswerNumbers[index] = answer;
		}
	}
	
	// 보안문자 출력 설정
	public void updateCaptchaQuestNumber(int index, int number)
	{
		if ((index >= 0) && (index < 4))
		{
			captchaQuestNumbers[index] = number;
		}
	}
	
	// 보안문자 입력패드 설정
	public void updateCaptchaNumberPad(int index, int number)
	{
		if ((index >= 0) && (index < 10))
		{
			captchaNumberPads[index] = number;
		}
	}
	
	// 보안문자 답변 가져오기
	public int getCaptchaAnswerNumber(int index)
	{
		if ((index >= 0) && (index < 4))
		{
			return captchaAnswerNumbers[index];
		}
		return -1; // 유효하지 않은 인덱스
	}
	
	// 보안문자 출력 가져오기
	public int getCaptchaQuestNumber(int index)
	{
		if ((index >= 0) && (index < 4))
		{
			return captchaQuestNumbers[index];
		}
		return -1; // 유효하지 않은 인덱스
	}
	
	// 보안문자 입력패드 가져오기
	public int getCaptchaNumberPad(int index)
	{
		if ((index >= 0) && (index < 10))
		{
			return captchaNumberPads[index];
		}
		return -1; // 유효하지 않은 인덱스
	}
	
	// 보안문자 입력횟수 설정
	public void updateCaptchaAnswerCount(int count)
	{
		captchaAnswerCount = count;
	}
	...
	// 컬렉터 옵션 추가
	public void addPAtk(int value)
	{
		_stat.addPAtk(value);
	}
	
	public void addPDef(int value)
	{
		_stat.addPDef(value);
	}
	
	public void addMAtk(int value)
	{
		_stat.addMAtk(value);
	}
	
	public void addMDef(int value)
	{
		_stat.addMDef(value);
	}
	...
	{
		_stat.addAccuracy(value);
	}
	
	public void addMAccuracy(int value)
	{
		_stat.addMAccuracy(value);
	}
	
	public void addPSkillCrtDmg(int value)
	{
		_stat.addPSkillCrtDmg(value);
	}
	
	public void addMCrtDmg(int value)
	{
		_stat.addMCrtDmg(value);
	}
	
	public void addPCrtChance(int value)
	{
		_stat.addPCrtChance(value);
	}
	
	public void addMCrtChance(int value)
	{
		_stat.addMCrtChance(value);
	}
	
	public void addResistParalyze(int value)
	{
		_stat.addResistParalyze(value);
	}
	
	public void addResistTurnstone(int value)
	{
		_stat.addResistTurnstone(value);
	}
	
	public void addResistDerangement(int value)
	{
		_stat.addResistDerangement(value);
	}
	
	public void addResistStun(int value)
	{
		_stat.addResistStun(value);
	}
	
	public void addResistHold(int value)
	{
		_stat.addResistHold(value);
	}
	
	public void addResistSleep(int value)
	{
		_stat.addResistSleep(value);
	}
	
	public void addResistBleed(int value)
	{
		_stat.addResistBleed(value);
	}
	
	public void addResistPoison(int value)
	{
		_stat.addResistPoison(value);
	}
	
	public void addResistSilence(int value)
	{
		_stat.addResistSilence(value);
	}
	
	public void addBonusHP(int value)
	{
		_stat.addBonusHP(value);
	}
	
	public void addBonusMP(int value)
	{
		_stat.addBonusMP(value);
	}
	
	public void addBonusCP(int value)
	{
		_stat.addBonusCP(value);
	}
	
	public void addBonusRusSpd(int value)
	{
		_stat.addBonusRusSpd(value);
	}
	
	public void addBonusPeVas(int value)
	{
		_stat.addBonusPeVas(value);
	}
	
	public void addBonusMeVas(int value)
	{
		_stat.addBonusMeVas(value);
	}
	
	public void addResistFire(int value)
	{
		_stat.addResistFire(value);
	}
	
	public void addResistWind(int value)
	{
		_stat.addResistWind(value);
	}
	
	public void addResistWater(int value)
	{
		_stat.addResistWater(value);
	}
	
	public void addResistEarth(int value)
	{
		_stat.addResistEarth(value);
	}
	
	public void addResistHoly(int value)
	{
		_stat.addResistHoly(value);
	}
	
	public void addResistDark(int value)
	{
		_stat.addResistDark(value);
	}
	
	public void addFirePower(int value)
	{
		_stat.addFirePower(value);
	}
	
	public void addWindPower(int value)
	{
		_stat.addWindPower(value);
	}
	
	public void addWaterPower(int value)
	{
		_stat.addWaterPower(value);
	}
	
	public void addEarthPower(int value)
	{
		_stat.addEarthPower(value);
	}
	
	public void addHolyPower(int value)
	{
		_stat.addHolyPower(value);
	}
	
	public void addDarkPower(int value)
	{
		_stat.addDarkPower(value);
	}
	
	public void addCPatk(int value)
	{
		_stat.addCPatk(value);
	}
	
	public void addMpRegen(int value)
	{
		_stat.addMpRegen(value);
	}
	
	public void applyCollectionRewards()
	{
		List<String> rewards = CollectionDatabaseManager.loadCollectionRewardsForPlayer(getAccountName());
		CollectionHandler.applyRewards(this, rewards);
		broadcastUserInfo();
	}
