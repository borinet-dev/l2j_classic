...
public abstract class Summon extends Playable
{
		...
		if (isSpawned())
		{
			...
			if (owner != null)
			{
				...
				if (isPet())
				{
					owner.setPetItems();
					owner.setPet(null);
					if ((getId() >= 1568) && (getId() <= 1573))
					{
						if (party != null)
						{
							for (Player member : owner.getParty().getMembers())
							{
								if (member.isAffectedBySkill(AAR_OF_SEDUCTION))
								{
									member.getEffectList().stopSkillEffects(SkillFinishType.REMOVED, AAR_OF_SEDUCTION.getSkill());
								}
							}
						}
						else
						{
							if (owner.isAffectedBySkill(AAR_OF_SEDUCTION))
							{
								owner.getEffectList().stopSkillEffects(SkillFinishType.REMOVED, AAR_OF_SEDUCTION.getSkill());
							}
						}
					}
				}
			...
			if (owner != null)
			{
				for (int itemId : owner.getAutoSoulShot())
				{
					...
					if ((handler != null) && handler.contains("야수"))
					{
	...
	public boolean canAttack(WorldObject target, boolean ctrlPressed)
	{
		...
		if (!Config.ENABLE_PK)
		{
			if ((!_owner.isInsideZone(ZoneId.PVP) || !_owner.getTarget().isInsideZone(ZoneId.PVP)) && _owner.getTarget().isPlayable())
			{
				final Clan leaderClan = _owner.getClan();
				final Clan targetClan = target.getActingPlayer().getClan();
				if (((leaderClan == null) || (targetClan == null) || !leaderClan.isAtWarWith(targetClan.getId())) && //
					!_owner.getActingPlayer().isInOlympiadMode() && //
					!_owner.getActingPlayer().isInSiege() && //
					!_owner.getActingPlayer().isOnEvent())
				{
					sendMessage("PVP존이 아닌곳에 있어서 공격할 수 없습니다.");
					sendPacket(ActionFailed.STATIC_PACKET);
					return false;
				}
			}
