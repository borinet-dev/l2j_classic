...
public class VillageMaster extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (actualCommand.equalsIgnoreCase("create_clan"))
		{
			...
			if (!cmdParams2.isEmpty() || !Util.isMatchingRegexp(cmdParams, Config.NAME_TEMPLATE) || !Util.isValidName(cmdParams))
			{
		...
		else if (actualCommand.equalsIgnoreCase("create_academy"))
		{
			...
			createSubPledge(player, cmdParams, null, Clan.SUBUNIT_ACADEMY, 3);
		}
		...
		else if (actualCommand.equalsIgnoreCase("create_royal"))
		{
			...
			createSubPledge(player, cmdParams, cmdParams2, Clan.SUBUNIT_ROYAL1, 5);
		}
		else if (actualCommand.equalsIgnoreCase("create_knight"))
		{
			...
			createSubPledge(player, cmdParams, cmdParams2, Clan.SUBUNIT_KNIGHT1, 5);
		}
		...
		else if (actualCommand.equalsIgnoreCase("cancel_clan_leader_change"))
		{
			...
			{
				msg.setHtml("<html><body>아직 제출된 혈맹주 위임 신청서가 없습니다!</body></html>");
			}
		...
		{
			if (!canChangeClass(player, getObjectId()))
			{
				return;
			}
			...
			switch (cmdChoice)
			{
				...
				case 1: // Add Subclass - Initial
					...
					{
						html.setFile(player, getSubClassMax());
						break;
					}
				...
				case 3: // Change/Cancel Subclass - Initial
					...
					/*
					 * if (player.isSubClassActive()) { player.sendMessage("서브클래스 취소를 하기위해서는 메인클래스 상태로 전환하세요."); html.setFile(player, "data/html/villagemaster/SubClass.htm"); break; }
					 */
				...
				case 4: // Add Subclass - Action (Subclass 4 x[x])
					...
					{
						html.setFile(player, getSubClassLowlvl());
						allowAddition = false;
						break;
					}
				...
				case 6: // Change/Cancel Subclass - Choice
					...
					/*
					 * if (player.isSubClassActive()) { player.sendMessage("메인클래스 상태에서 시도하시기 바랍니다."); html.setFile(player, "data/html/villagemaster/SubClass.htm"); break; }
					 */
	...
	private static boolean canChangeClass(Player player, int npcId)
	{
		if (player.isOnEvent())
		{
	...
	protected String getSubClassMax()
	{
		return "data/html/villagemaster/SubClass_Max.htm";
	}
	
	protected String getSubClassLowlvl()
	{
		return "data/html/villagemaster/SubClass_Lowlvl.htm";
	}
	...
	private void createSubPledge(Player player, String clanName, String leaderName, int pledgeType, int minClanLvl)
	{
		...
		if (clan.getLevel() < minClanLvl)
		{
			...
			{
				player.sendMessage("혈맹 아카데미를 창설하려면 혈맹레벨이 " + minClanLvl + "레벨 이상이어야 합니다.");
		...
		if (!Util.isMatchingRegexp(clanName, Config.NAME_TEMPLATE) || !Util.isAlphaNumeric(clanName) || !Util.isValidName(clanName) || (2 > clanName.length()))
		{
			player.sendPacket(SystemMessageId.CLAN_NAME_IS_INVALID);
			return;
		}
	...
	private void renameSubPledge(Player player, int pledgeType, String pledgeName)
	{
		...
		if (!Util.isMatchingRegexp(pledgeName, Config.NAME_TEMPLATE) || !Util.isAlphaNumeric(pledgeName) || !Util.isValidName(pledgeName) || (2 > pledgeName.length()))
		{
			player.sendPacket(SystemMessageId.CLAN_NAME_IS_INVALID);
			return;
		}
	...
	private void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if (leaderName.matches(".*[\\uAC00-\\uD7A3]+.*"))
		{
			if ((leaderName.length() < 1) || (leaderName.length() > 8))
			{
