...
public class Attackable extends Npc
{
		...
		try
		{
			...
			if (!rewards.isEmpty())
			{
				for (DamageDoneInfo reward : rewards.values())
				{
					...
					{
						usingPole = attacker.getActiveWeaponItem().getItemType() == WeaponType.POLE;
					}
					...
					if (attackerParty == null)
					{
						...
						if (isInSurroundingRegion(attacker))
						{
							...
							exp *= Config.ENABLE_POLE_RATE ? (usingPole ? Config.POLE_EXPSP_RATE : 1) : 1;
							sp *= Config.ENABLE_POLE_RATE ? (usingPole ? Config.POLE_EXPSP_RATE : 1) : 1;
	...
	public void onRespawn()
	{
		...
		if (Config.CHAMPION_ENABLE && isMonster() && !isQuestMonster() && !getTemplate().isUndying() && !_isRaid && !_isRaidMinion && (Config.CHAMPION_FREQUENCY > 0) && (getLevel() >= Config.CHAMP_MIN_LEVEL) && (getLevel() <= Config.CHAMP_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)))
		{
			...
			{
				if (getTemplate().isSevenSignsParty() || getTemplate().isSevenSignsSolo() || getTemplate().RandomBoss() || getTemplate().EventMonsters() || (getId() == 31468))
				{
					_champion = false;
				}
				else
