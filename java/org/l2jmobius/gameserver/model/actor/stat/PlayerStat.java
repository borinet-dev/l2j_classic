...
public class PlayerStat extends PlayableStat
{
	...
	public void addExpAndSp(double addToExpValue, double addToSpValue, boolean useBonuses, boolean premium)
	{
		final Player player = getActiveChar();
		...
		// Remove Karma when the player kills L2Monster
		if (!player.isCursedWeaponEquipped() && (addToSp > 0) && (player.getReputation() < 0) && (!player.isInsideZone(ZoneId.PVP)))
		{
			int toDecrease = (Config.KARMA_LOST_MIN / 10) + (player.getPkKills() * Config.KARMA_SP_DIVIDER);
			int setKarma = player.getReputation() + Rnd.get(toDecrease / 2, toDecrease * 2);
			player.setReputation(setKarma > 0 ? 0 : setKarma);
		}
		
		// Premium rates
		if (premium && player.hasPremiumStatus())
		{
			addToExp *= Config.PREMIUM_RATE_XP;
			addToSp *= Config.PREMIUM_RATE_SP;
		}
		
		final double baseExp = addToExp * Config.CUSTOM_RATE_XPSP;
		final double baseSp = addToSp * Config.CUSTOM_RATE_XPSP;
		double bonusExp = 1;
		double bonusSp = 1;
		if (useBonuses)
		{
		...
		{
			player.addQuickVar("LastExp", System.currentTimeMillis());
	...
	{
		return Config.ENABLE_VITALITY ? (getVitalityPoints() > 0 ? getMul(Stat.VITALITY_EXP_RATE, Config.RATE_VITALITY_EXP_MULTIPLIER) : Config.RATE_VITALITY_EXP_MULTIPLIER) : 1.0;
	}
	...
	public void recalculateStats(boolean broadcast)
	{
		if (!getActiveChar().isChangingClass() && !getActiveChar().isUsingSkillItem())
		{
			if (broadcast)
			{
				if (_recalculateTask != null)
				{
					super.recalculateStats(false);
					return;
				}
				
				_recalculateTask = ThreadPool.schedule(() ->
				{
					super.recalculateStats(broadcast);
					_recalculateTask = null;
				}, 50);
			}
			else
			{
