...
public class Quest extends AbstractScript implements IIdentifiable
{
	...
	public String getHtm(Player player, String fileName)
	{
		...
		if (content == null)
		{
			...
			if (content == null)
			{
				...
				if (content == null)
				{
					content = hc.getHtm(player, "data/html/no_html_file.html");
	...
	{
		addCondStart(p -> p.getOriginRace() == race, html);
	}
	...
	{
		addCondStart(p -> p.getOriginRace() == race, pairs);
	}
	...
	{
		addCondStart(p -> p.isMainClassActive() && !p.isDualClassActive(), html);
	}
	...
	{
		addCondStart(p -> p.isMainClassActive() && !p.isDualClassActive(), pairs);
	}
