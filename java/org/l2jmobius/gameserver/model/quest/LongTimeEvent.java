...
public class LongTimeEvent extends Quest
{
	...
	public static boolean _isSpawned;
		...
		new IXmlReader()
		{
			...
			public void parseDocument(Document doc, File f)
			{
				...
				switch (period.length())
				{
					...
					{
						// MM dd-MM dd
						_eventPeriod = DateRange.parse(activePeriod, new SimpleDateFormat("MM dd yyyy", Locale.KOREAN));
						break;
					}
					case 21:
					{
						// yyyy MM dd-yyyy MM dd
						_eventPeriod = DateRange.parse(period, new SimpleDateFormat("yyyy MM dd", Locale.KOREAN));
						break;
					}
					case 23:
					{
						// MM dd HH mm-MM dd HH mm
						_eventPeriod = DateRange.parse(activePeriod, new SimpleDateFormat("MM dd HH mm yyyy", Locale.KOREAN));
						break;
					}
					case 33:
					{
						// yyyy MM dd HH mm-yyyy MM dd HH mm
						_eventPeriod = DateRange.parse(period, new SimpleDateFormat("yyyy MM dd HH mm", Locale.KOREAN));
						break;
					}
				}
							...
							for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
							{
								if (d.getNodeName().equalsIgnoreCase("add"))
								{
									try
									{
										...
										final Node isDropNode = d.getAttributes().getNamedItem("isDrop");
										final boolean dropTrue = ((isDropNode == null) || isDropNode.getNodeValue().equals("false")) ? false : true;
										...
										_dropList.add(new EventDropHolder(itemId, minCount, maxCount, finalChance, minLevel, maxLevel, monsterIds, dropTrue));
									}
							...
							for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
							{
								if (d.getNodeName().equalsIgnoreCase("add"))
								{
									...
									if ((msgType != null) && (msgText != null))
									{
										...
										else if (msgType.equalsIgnoreCase("onStart"))
										{
											_onStartMsg = msgText;
										}
	...
	protected void startEvent()
	{
		...
		// Start enter announcement.
		if (!_onStartMsg.isEmpty())
		{
			// Send message on begin.
			Broadcast.announcementMsg(_onStartMsg);
		}
	...
	protected void stopEvent()
	{
		...
		if (!_endMsg.isEmpty())
		{
			Broadcast.announcementMsg(_endMsg);
		}
		...
		if (month != Calendar.JUNE)
		{
			...
			{
				player.stopSkillEffects(SkillFinishType.REMOVED, 30279);
				player.getVariables().remove("FREYA_BUFF");
			}
			try (Connection con = DatabaseFactory.getConnection();
				Statement statement = con.createStatement())
			{
				statement.executeUpdate("DELETE FROM character_skills_save WHERE skill_id = '30279';");
				statement.executeUpdate("DELETE FROM character_variables WHERE var = 'FREYA_BUFF';");
			}
		...
		if (!_destroyItemsOnEnd.isEmpty())
		{
			for (int itemId : _destroyItemsOnEnd)
			{
				...
				for (Player player : World.getInstance().getPlayers())
				{
					...
					if (item != null)
					{
						player.destroyItemByItemId(_eventName, itemId, item.getCount(), player, true);
					}
				}
				// Update database.
				try (Connection con = DatabaseFactory.getConnection();
