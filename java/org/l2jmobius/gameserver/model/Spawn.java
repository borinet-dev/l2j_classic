...
public class Spawn extends Location implements IIdentifiable, INamable
{
	...
	private Npc initializeNpc(Npc npc)
	{
		...
		final NpcTemplate template = NpcData.getInstance().getTemplate(npc.getId());
		// 일반 NPC로 간주할 조건을 여기에 추가합니다.
		if (template.getType().equals("Folk") || template.getType().equals("Chest") || template.getType().equals("CommissionManager") //
			|| template.getType().equals("ControlTower") || template.getType().equals("Door") || template.getType().equals("EventMonster") //
			|| template.getType().equals("Merchant") || template.getType().equals("RaceManager") //
			|| template.getType().equals("SchemeBuffer") || template.getType().equals("Teleporter") //
			|| getTemplate().isSevenSignsParty() || getTemplate().isSevenSignsSolo() //
			|| getTemplate().RandomBoss() || getTemplate().EventMonsters() //
			|| getTemplate().isUndying() //
			|| WalkingManager.getInstance().isTargeted(npc) //
			|| npc.isQuestMonster() //
			|| (npc instanceof GrandBoss) || (npc instanceof RaidBoss) || npc.isRaidMinion() //
			|| npc.isFlying() //
		...
		if (isRandomSpawn && Config.ENABLE_RANDOM_MONSTER_SPAWNS && (getHeading() != -1) && npc.isMonster() && (getInstanceId() == 0) && (water == null))
		{
			...
			Location safeSpawnLocation = BorinetUtil.getInstance().findSafeLocation(newlocx, newlocy, newlocz, 200);
		...
		if (npc.isQuestMonster() || npc.isRaid() || npc.isRaidMinion() || Config.MOBS_LIST_NOT_RANDOM.contains(npc.getId()))
		{
			// Do not correct Z distances greater than 300.
			final int geoZ = GeoEngine.getInstance().getHeight(newlocx, newlocy, newlocz);
			if (Util.calculateDistance(newlocx, newlocy, newlocz, newlocx, newlocy, geoZ, true, true) < 300)
	...
	public void setRespawnDelay(int delay, int randomInterval)
	{
		int respawnTime = 0;
		int spawnDelay = 0;
		int times = Config.MONSTER_RESPAWN_REDUCED_TIMES;
		if (delay > 0)
		{
			if (delay < 120)
			{
				int spawnTime = delay * 2;
				respawnTime = ((spawnTime * times) / 100);
				spawnDelay = delay - respawnTime;
				final int minDelay = Config.ENABLE_REDUCED_TIMES ? spawnDelay - (randomInterval / 2) : spawnTime - randomInterval;
				final int maxDelay = Config.ENABLE_REDUCED_TIMES ? spawnDelay - (randomInterval / 2) : spawnTime + randomInterval;
			...
			{
				respawnTime = ((delay * times) / 100);
				spawnDelay = delay - respawnTime;
				final int minDelay = Config.ENABLE_REDUCED_TIMES ? spawnDelay - (randomInterval / 2) : delay - randomInterval;
				final int maxDelay = Config.ENABLE_REDUCED_TIMES ? spawnDelay - (randomInterval / 2) : delay + randomInterval;
