...
public class CursedWeapon implements INamable
{
	...
	public void endOfLife()
	{
		if (_isActivated)
		{
			if ((_player != null) && _player.isOnline())
			{
				...
				// Destroy
				// _player.getAppearance().setVisibleTitle(null);
				setNameTitle();
			...
			else
			{
				...
				try (Connection con = DatabaseFactory.getConnection();
					...
					PreparedStatement ps = con.prepareStatement("UPDATE characters SET char_name=?, reputation=?, pkkills=? WHERE charId=?"))
				{
					// Delete the item
	...
	public void cursedOnLogin()
	{
		...
		{
			_player.getClan().broadcastClanStatus();
		}
		_player.setTitle("혈검");
		_player.getAppearance().setTitleColor(Integer.decode("0x0000FF"));
		_player.setName("아카마나프");
		doTransform();
		giveSkill();
		_player.broadcastUserInfo();
	...
	public void activate(Player player, Item item)
	{
		...
		_player.getVariables().set("저주받은무기_호칭", _player.getTitle());
		_player.getVariables().set("저주받은무기_호칭색상", _player.getAppearance().getTitleColor());
		_player.getVariables().set("저주받은무기_이름", _player.getName());
		_playerName = _player.getVariables().getString("저주받은무기_이름", "");
	...
	public void saveData()
	{
		try (Connection con = DatabaseFactory.getConnection();
			...
			PreparedStatement ps = con.prepareStatement("INSERT INTO cursed_weapons (itemId, charId, charName, playerReputation, playerPkKills, nbKills, endTime) VALUES (?, ?, ?, ?, ?, ?, ?)"))
		{
			...
			if (_isActivated)
			{
				...
				ps.setString(3, _playerName);
				ps.setInt(4, _playerReputation);
				ps.setInt(5, _playerPkKills);
				ps.setInt(6, _nbKills);
				ps.setLong(7, _endTime);
				ps.executeUpdate();
			}
		}
	...
	{
		_player.setTitle(_player.getVariables().getString("저주받은무기_호칭", ""));
		_player.getAppearance().setTitleColor(_player.getVariables().getInt("저주받은무기_호칭색상", 0));
		_player.setName(_player.getVariables().getString("저주받은무기_이름", ""));
		
		if (_player.getClan() != null)
		{
			_player.getClan().broadcastClanStatus();
		}
		_player.getVariables().remove("저주받은무기_호칭");
		_player.getVariables().remove("저주받은무기_호칭색상");
		_player.getVariables().remove("저주받은무기_이름");
	}
