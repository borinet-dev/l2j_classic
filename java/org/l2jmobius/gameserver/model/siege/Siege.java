...
public class Siege implements Siegable
{
	...
	public class ScheduleStartSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				else if ((timeRemaining <= 86400000) && (timeRemaining > 13600000) && (_castle.getSiegeDate().getTimeInMillis() > 0))
				{
	...
	{
		if (_castle.getSiegeDate().getTimeInMillis() > 0)
		{
			if (_isInProgress)
			{
	...
	public void registerDefender(Player player, boolean force)
	{
		if (_castle.getOwnerId() <= 0)
		{
			player.sendMessage(_castle.getName() + "은 NPC 소유이기 때문에 수성측으로 참가신청을 할 수 없습니다.");
			return;
		}
		if (_castle.getSiegeDate().getTimeInMillis() < System.currentTimeMillis())
		{
			player.sendMessage(_castle.getName() + "은 공성일시가 미정이기 때문에 수성측으로 참가신청을 할 수 없습니다.");
			return;
		}
		if (force)
		{
	...
	public void startAutoTask()
	{
		...
		String date = BorinetUtil.dataDateFormatKor.format(new Date(_castle.getSiegeDate().getTimeInMillis()));
		LOGGER.info(_castle.getName() + ": " + date);
		loadSiegeClan();
	...
	private boolean checkIfCanRegister(Player player, byte typeId)
	{
		...
		else if (_castle.getSiegeDate().getTimeInMillis() < System.currentTimeMillis())
		{
			player.sendMessage(_castle.getName() + "은 공성일시가 미정이기 때문에 공성측에 참가신청을 할 수 없습니다.");
		}
	...
	private void setNextSiegeDate()
	{
		...
		{
			calendar.add(Calendar.WEEK_OF_YEAR, Config.SIEGE_NEXT_WEEK);
		}
		
		if ((CastleManager.getInstance().getSiegeDates(calendar.getTimeInMillis()) < holder.getMaxConcurrent()) && (_castle.getSiegeDate().getTimeInMillis() > 0))
		{
