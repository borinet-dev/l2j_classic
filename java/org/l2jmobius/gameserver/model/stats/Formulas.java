...
public class Formulas
{
	...
	public static double calcBlowDamage(Creature attacker, Creature target, Skill skill, boolean backstab, double power, byte shld, boolean ss)
	{
		...
		// Critical
		double baseCriticalDamage = attacker.getStat().getValue(Stat.CRITICAL_DAMAGE, 1);
		double bonusCriticalDamage = attacker.getStat().getPSkillCrtDmg() / 100.0;
		final double criticalMod = baseCriticalDamage + (baseCriticalDamage * bonusCriticalDamage);
	...
	public static double calcMagicDam(Creature attacker, Creature target, Skill skill, double mAtk, double power, double mDef, boolean sps, boolean bss, boolean mcrit)
	{
		...
		if (Rnd.chance(target.getStat().getSkillEvasionTypeValue(skill.getMagicType())) && (target.getEffectList().getBuffInfoBySkillId(446) != null))
		{
			calcSkillEvasion(attacker, target, skill);
			damage = 0;
		}
		else
		{
			boolean Fail = false;
			int Srate = Rnd.get(100);
			if (Config.ALT_GAME_MAGICFAILURES && (Srate < 23))
			{
				Fail = true;
			}
			
			// Failure calculation
			if (Fail && !calcMagicSuccess(attacker, target, skill))
			{
	...
	public static boolean calcCrit(double rateValue, Creature creature, Creature target, Skill skill)
	{
		...
		if (skill != null)
		{
			...
			double baseCritChance = creature.getStat().getMul(Stat.CRITICAL_RATE_SKILL, 1);
			double bonusCritChance = creature.getStat().getPCrtChance() / 100.0;
			final double rateBonus = baseCritChance + (baseCritChance * bonusCritChance);
	...
	public static double calcCritDamage(Creature attacker, Creature target, Skill skill)
	{
		...
		if (skill != null)
		{
			if (skill.isMagic())
			{
				...
				double baseCriticalDamage = attacker.getStat().getValue(Stat.MAGIC_CRITICAL_DAMAGE, 1);
				double bonusCriticalDamage = attacker.getStat().getMCrtDmg() / 100.0;
				criticalDamage = baseCriticalDamage + (baseCriticalDamage * bonusCriticalDamage);
			...
			{
				double baseCriticalDamage = attacker.getStat().getValue(Stat.PHYSICAL_SKILL_CRITICAL_DAMAGE, 1);
				double bonusCriticalDamage = attacker.getStat().getPSkillCrtDmg() / 100.0;
				criticalDamage = baseCriticalDamage + (baseCriticalDamage * bonusCriticalDamage);
		...
		else
		{
			...
			double baseCriticalDamage = attacker.getStat().getValue(Stat.CRITICAL_DAMAGE, 1);
			double bonusCriticalDamage = attacker.getStat().getPCrtDmg() / 100.0;
			criticalDamage = (baseCriticalDamage + (baseCriticalDamage * bonusCriticalDamage)) * attacker.getStat().getPositionTypeValue(Stat.CRITICAL_DAMAGE, Position.getPosition(attacker, target));
		...
		final Weapon wpn = attacker.getActiveWeaponItem();
		if (wpn != null)
		{
			if (wpn.getItemType() == WeaponType.DUALBLUNT)
			{
				crtDam *= 0.85;
			}
			else if ((wpn.getItemType() == WeaponType.DUALBLUNT) || (wpn.getItemType() == WeaponType.BOW))
			{
				crtDam *= 0.8;
			}
			if (wpn.getCrystalType() == CrystalType.R)
			{
				if ((wpn.getItemType() == WeaponType.DUALDAGGER) || (wpn.getItemType() == WeaponType.DUALFIST) || (wpn.getItemType() == WeaponType.BIGSWORD))
				{
					crtDam *= 1.2;
				}
				else if ((wpn.getItemType() == WeaponType.SWORD) || (wpn.getItemType() == WeaponType.DAGGER) || (wpn.getItemType() == WeaponType.DUAL))
				{
					crtDam *= 1.1;
				}
			}
		...
		// return Math.max(1, 2 + ((criticalDamage - defenceCriticalDamage) * balanceMod));
		// return (criticalDamage - defenceCriticalDamage) * balanceMod;
		return (2 * criticalDamage * defenceCriticalDamage * balanceMod) * crtDam;
	}
	...
	public static boolean calcMagicAffected(Creature actor, Creature target, Skill skill)
	{
		...
		if (skill.isDebuff())
		{
			if (target.getEffectList().getBuffInfoBySkillId(30257) != null)
			{
				return false;
			}
	...
	public static boolean calcEffectSuccess(Creature attacker, Creature target, Skill skill)
	{
		...
		if ((skill.getId() == 522) && Rnd.chance(70))
		{
			final SystemMessage sm = new SystemMessage(SystemMessageId.C1_HAS_RESISTED_YOUR_S2);
			sm.addString(target.getName());
			sm.addSkillName(skill);
		...
		if (skill.isDebuff() && (target.getEffectList().getBuffInfoBySkillId(30257) != null))
		{
			final SystemMessage sm = new SystemMessage(SystemMessageId.C1_HAS_RESISTED_YOUR_S2);
			sm.addString(target.getName());
			sm.addSkillName(skill);
		...
		if (target.isPlayer())
		{
			switch (skill.getAbnormalType())
			{
				case PARALYZE:
				{
					int per = ((Player) target).getParalyzeResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case TURN_STONE:
				{
					int per = ((Player) target).getTurnStoneResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case TURN_FLEE:
				{
					int per = ((Player) target).getDerangementResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case STUN:
				{
					int per = ((Player) target).getStunResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case ROOT_MAGICALLY:
				{
					int per = ((Player) target).getHoldResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case SLEEP:
				{
					int per = ((Player) target).getSleepResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case BLEEDING:
				{
					int per = ((Player) target).getBleedResist();
					if (per >= 100)
					{
						isImmune = true;
					}
				...
				case POISON:
				{
					int per = ((Player) target).getPoisonResist();
					if (per >= 100)
					{
						isImmune = true;
					}
		...
		if (((finalRate <= Rnd.get(100)) && (target != attacker)) || isImmune)
		{
			final SystemMessage sm = new SystemMessage(SystemMessageId.C1_HAS_RESISTED_YOUR_S2);
			sm.addString(target.getName());
			sm.addSkillName(skill);
	...
	public static double calcManaDam(Creature attacker, Creature target, Skill skill, double power, byte shld, boolean sps, boolean bss, boolean mcrit, double critLimit)
	{
		...
		boolean nonBlessed = attacker.isChargedShot(ShotType.SPIRITSHOTS);
		boolean blessed = attacker.isChargedShot(ShotType.BLESSED_SPIRITSHOTS);
		
		double sapphireBonus = 0;
		if (attacker.isPlayer() && (attacker.getActingPlayer().getActiveSapphireJewel() != null) && (weapon != null))
		{
			if (nonBlessed || blessed)
			{
				sapphireBonus = attacker.getActingPlayer().getActiveSapphireJewel().getBonus();
			}
		}
	...
	public static double calcAttributeBonus(Creature attacker, Creature target, Skill skill)
	{
		...
		if (skill != null)
		{
			if (skill.getAttributeType() == AttributeType.NONE)
			{
				return 1;
			}
	...
	public static double calculatePvpPveBonus(Creature attacker, Creature target, Skill skill, boolean crit)
	{
		...
		if (attacker.isPlayable() && target.isPlayable())
		{
			...
			if (skill != null)
			{
				if (skill.isMagic())
				{
					...
					if (attacker.isPlayable() && (attacker.getActiveWeaponItem() != null) && (attacker.getActiveWeaponItem().isMagicWeapon()))
					{
						if (attacker.getActiveWeaponItem().getCrystalType() == CrystalType.R)
						{
							if (attacker.getActiveWeaponInstance().getWeaponItem().isBlessed())
							{
								pvpAttack *= 1.3;
							}
							else
						...
						if (attacker.getActiveWeaponInstance().getWeaponItem().getBodyPart() == ItemTemplate.SLOT_LR_HAND)
						{
							pvpAttack *= 1.15;
						}
					}
		...
		// PvE Bonus
		if (attacker.isPlayable() && target.isNpc())
		{
			double pveAttack;
			...
			if (skill != null)
			{
				if (skill.isMagic())
				{
					...
					if (attacker.isPlayable() && (attacker.getActiveWeaponItem() != null) && attacker.getActiveWeaponItem().isMagicWeapon())
					{
						pveAttack *= Config.MAGICWEAPON_ATTACK_DAMAGE_TO_MONSTER;
						if (attacker.getActiveWeaponItem().getCrystalType() == CrystalType.R)
						{
							if (attacker.getActiveWeaponInstance().getWeaponItem().isBlessed())
							{
				...
				else
				{
					...
					if (attacker.isPlayable() && (attacker.getActiveWeaponItem() != null))
					{
						WeaponType weaponType = attacker.getActiveWeaponItem().getItemType();
						if (!attacker.getActiveWeaponItem().isMagicWeapon())
						{
							if (((Npc) target).getTemplate().isGolem())
							{
								if (weaponType == WeaponType.BLUNT)
								{
									pveAttack *= Config.BLUNT_ATTACK_DAMAGE_TO_GOLEM;
									pveRaidAttack *= Config.BLUNT_ATTACK_DAMAGE_TO_GOLEM;
								}
								if (weaponType == WeaponType.DUALBLUNT)
								{
									pveAttack *= Config.DUALBLUNT_ATTACK_DAMAGE_TO_GOLEM;
									pveRaidAttack *= Config.DUALBLUNT_ATTACK_DAMAGE_TO_GOLEM;
								}
								if ((weaponType == WeaponType.DAGGER) || (weaponType == WeaponType.BOW) || (weaponType == WeaponType.DUALDAGGER))
								{
									pveAttack *= Config.GOLEM_ATTACK_DAMAGE_TO_WEAK;
									pveRaidAttack *= Config.GOLEM_ATTACK_DAMAGE_TO_WEAK;
								}
							}
							else
							{
								...
								{
									if (ClassId.isBow(classId))
									{
										pveAttack *= Config.BOW_ATTACK_DAMAGE_TO_MONSTER;
										pveRaidAttack *= Config.BOW_ATTACK_DAMAGE_TO_MONSTER;
									}
									else
									{
										pveAttack *= 0.8;
								...
								{
									pveAttack *= Config.BLUNT_ATTACK_DAMAGE_TO_MONSTER;
									pveRaidAttack *= Config.BLUNT_ATTACK_DAMAGE_TO_MONSTER;
								}
								else if (weaponType == WeaponType.DUALBLUNT)
								{
									pveAttack *= Config.DUALBLUNT_ATTACK_DAMAGE_TO_MONSTER;
									pveRaidAttack *= Config.DUALBLUNT_ATTACK_DAMAGE_TO_MONSTER;
								}
								else if (weaponType == WeaponType.POLE)
								{
									boolean isForgottenTemplate = ((Npc) target).getTemplate().isForgotten();
									boolean isMonasteryTemplate = ((Npc) target).getTemplate().isMonastery();
									boolean isElvenForestTemplate = ((Npc) target).getTemplate().isElvenFortress();
									boolean isBeastFarmTemplate = ((Npc) target).getTemplate().isBeastFarm();
									boolean isMithrilMineTemplate = ((Npc) target).getTemplate().isMithrilMine();
									
									if (isForgottenTemplate || isMonasteryTemplate || isElvenForestTemplate || isBeastFarmTemplate || isMithrilMineTemplate)
									{
										pveAttack *= Config.POLE_ATTACK_DAMAGE_TO_FORGOTTEN;
									}
									else
									{
										pveAttack *= Config.POLE_ATTACK_DAMAGE_TO_MONSTER;
									}
								}
								else if ((weaponType == WeaponType.SWORD) || (weaponType == WeaponType.DUAL))
								{
									pveAttack *= 1.08;
									pveRaidAttack *= 1.08;
								}
								else if (weaponType == WeaponType.BIGSWORD)
								{
									pveAttack *= 0.9;
									pveRaidAttack *= 0.9;
								}
	...
	public static double calcAttackElementalDamage(Creature attacker, Creature target)
	{
		if (!attacker.isPlayer())
		{
			return 0;
		...
		{
			double attackerValue = attackerPlayer.getAttackElementValue(attribute);
			double targetValue = target.getDefenseElementValue(attribute);
			attack += (attackerValue - targetValue);
		}
