...
public class BaseStatFinalizer implements IStatFunction
{
	...
	public double calc(Creature creature, OptionalDouble base, Stat stat)
	{
		...
		if (creature.isPlayer())
		{
			...
			// Henna calculation
			baseValue += player.getHennaValue(BaseStat.valueOf(stat)) > 9 ? 9 : player.getHennaValue(BaseStat.valueOf(stat));
			...
			switch (stat)
			{
				case STAT_STR:
				{
					...
					// player.getVariables().getInt(PlayerVariables.STAT_STR, 0);
					break;
				}
				case STAT_CON:
				{
