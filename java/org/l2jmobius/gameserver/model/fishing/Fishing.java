...
public class Fishing
{
	...
	private void castLine()
	{
		...
		{
			_player.sendMessage("낚시를 할 수 없습니다.");
			_player.sendPacket(ActionFailed.STATIC_PACKET);
			stopFishing(FishingEndType.ERROR);
			return;
		}
		
		if (_player.isInVehicle() || (_player.isInBoat() && _player.isOnBoat()))
		{
			_player.sendMessage("정기선 탑승 상태에서는 낚시를 할 수 없습니다.");
			_player.sendPacket(ActionFailed.STATIC_PACKET);
			stopFishing(FishingEndType.ERROR);
			return;
		}
		...
		if (_player.isTransformed() || (_player.isInBoat() && _player.isOnBoat()))
		{
	...
	public void reelInWithReward()
	{
		...
		if (baitData == null)
		{
			...
			// LOGGER.warning(_player + " is fishing with unhandled bait: " + _player.getInventory().getPaperdollItem(Inventory.PAPERDOLL_LHAND));
			return;
		}
