...
public class NpcData implements IXmlReader
{
	...
	public synchronized void load()
	{
		...
		// LOGGER.info(getClass().getSimpleName() + ": " + _npcs.size() + " 기의 NPC를 로드하였습니다.");
		if (Config.CUSTOM_NPC_DATA)
		{
			// final int npcCount = _npcs.size();
			parseDatapackDirectory("data/stats/npcs/custom", true);
			// LOGGER.info(getClass().getSimpleName() + ": " + (_npcs.size() - npcCount) + " 기의 custom NPC를 로드하였습니다.");
		}
	}
	
	public void reload()
	{
		LOGGER.info("NPC 리로드 시작하는 중...");
		_masterMonsterIDs.clear();
		_npcs.clear();
		load();
	}
			...
			if ("list".equalsIgnoreCase(node.getNodeName()))
			{
				for (Node listNode = node.getFirstChild(); listNode != null; listNode = listNode.getNextSibling())
				{
					if ("npc".equalsIgnoreCase(listNode.getNodeName()))
					{
						...
						set.set("name", NpcNameTable.getInstance().getNpcNameKor(npcId));
						...
						if (Config.BOSS_DROP_ENABLED)
						{
							if (type.contains("RaidBoss") && (level >= Config.BOSS_DROP_MIN_LEVEL) && (level <= Config.BOSS_DROP_MAX_LEVEL))
							{
								if (dropLists == null)
								{
									dropLists = new ArrayList<>();
								}
								dropLists.addAll(Config.RAIDBOSS_DROP_LIST);
							}
							if (type.contains("GrandBoss"))
							{
								if (dropLists == null)
								{
									dropLists = new ArrayList<>();
								}
								dropLists.addAll(Config.GRANDBOSS_DROP_LIST);
							}
						}
