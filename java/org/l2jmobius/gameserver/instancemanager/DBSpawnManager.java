...
public class DBSpawnManager
{
	...
	private void scheduleSpawn(int npcId)
	{
		...
		if (npc != null)
		{
			...
			_npcs.put(npcId, npc);
			npc.broadcastPacket(new ExShowScreenMessage("레이드 보스가 스폰되었습니다! - [" + npc.getName() + "]", 2, 5000));
			// LOGGER.info("레이드 보스 스폰 - " + npc.getName());
		}
	...
	public void updateStatus(Npc npc, boolean isNpcDead)
	{
		...
		if (isNpcDead)
		{
			...
			{
				respawnMinDelay = npc.getSpawn().getRespawnMinDelay() * Config.RAID_MIN_RESPAWN_TIME;
				respawnMaxDelay = npc.getSpawn().getRespawnMaxDelay() * Config.RAID_MAX_RESPAWN_TIME;
			...
			{
				// LOGGER.info("레이드 보스 - [" + npc.getName() + "] 사망. 다음 리스폰 시간 - " + Util.formatDate(new Date(respawnTime), "yyyy.MM.dd HH:mm"));
