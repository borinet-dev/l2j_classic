...
public class InstanceManager implements IXmlReader
{
	...
	public void load()
	{
		...
		// LOGGER.info(getClass().getSimpleName() + ": " + _instanceNames.size() + " 개의 인스턴트 던전이름을 로드하였습니다.");
		// Load instance templates
		_instanceTemplates.clear();
		parseDatapackDirectory("data/instances", true);
		// LOGGER.info(getClass().getSimpleName() + ": " + _instanceTemplates.size() + " 개의 인스턴트 던전을 로드하였습니다.");
		// Load player's reenter data
		_PlayerTimes.clear();
		restoreInstanceTimes();
		// LOGGER.info(getClass().getSimpleName() + ": " + _PlayerTimes.size() + " 명의 플레이어에 대해 로드된 인스턴스 재진입 시간.");
	}
