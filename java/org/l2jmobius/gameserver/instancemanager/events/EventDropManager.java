...
public class EventDropManager
{
	...
	public void doEventDrop(Creature attacker, Attackable attackable)
	{
		...
		final NpcTemplate npcTemplate = NpcData.getInstance().getTemplate(attackable.getId());
		// Event items drop only for players.
		if ((attacker == null) || !attacker.isPlayable() || attackable.isFakePlayer() || !npcTemplate.getType().equals("Monster"))
		{
			return;
		}
		...
		if (Config.WATERMELON_EVENT_ENABLED && BorinetTask.WeekendCheck() && Rnd.chance(Config.WATERMELON_EVENT_RATE))
		{
			player.doAutoLoot(attackable, 6391, 1);
		}
		...
		for (List<EventDropHolder> eventDrops : EVENT_DROPS.values())
		{
			DROPS: for (EventDropHolder drop : eventDrops)
			{
				...
				if ((monsterLevel >= drop.getMinLevel()) && (monsterLevel <= drop.getMaxLevel()) && (Rnd.get(100d) < drop.getChance()))
				{
					...
					if (item.hasExImmediateEffect() && !Config.AUTO_LOOT_HERBS)
					{
						attackable.dropItem(player, itemId, itemCount); // Drop the item on the ground.
					}
					else if (drop.isEventDrop())
					{
						attackable.dropItem(player, itemId, itemCount); // Drop the item on the ground.
					}
					else if (Config.AUTO_LOOT_ITEM_IDS.contains(itemId) || Config.AUTO_LOOT || attackable.isFlying())
					{
